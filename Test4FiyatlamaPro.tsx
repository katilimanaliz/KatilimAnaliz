  ...
  const pdfOlusturVePaylas = async () => {
    ...
    try {
      ...
      div.innerHTML = `
        ...
        <div style="padding:12px 24px;background:#F9F9FB;border-top:1px solid #E5E7EB;margin-top:4px;">
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      ...
      {plan?.rows&&<>
        ...
        <Card><SecTitle>Ödeme Planı</SecTitle><OdemePlanTablosu plan={plan.rows.map(r=>({...r,karPayi:r.karPayi}))}/></Card>
      </>}
    </div>
  );
}
  ...
  useEffect(()=>{
    const loadKurlar = async () => {
      ...
      if(d){
        ...
        if(altin?.XAU_TRY_gram) liste.push({kod:"Altın (g)", try: altin.XAU_TRY_gram, altin:true});
        if(altin?.XAG_TRY_gram) liste.push({kod:"Gümüş (g)", try: altin.XAG_TRY_gram, altin:true});
        setKurlar(liste);
        setKurYuklendi(true);
      }
    };
              ...
              <div className="ticker-inner">
                {[...kurlar,...kurlar].map((k,i)=>(
                  <span key={i} style={{
                    ...
                    <span style={{color:"rgba(255,255,255,0.45)",fontSize:10}}>{k.altin ? k.kod : k.kod+"/TRY"}</span>
                    <span style={{color:kurYuklendi?(k.altin?"#FFD60A":"#4ADE80"):"rgba(255,255,100,0.7)",fontFamily:"monospace"}}>{typeof k.try==="number"?fmtN(k.try,k.altin?2:4):k.try}</span>
                  </span>
                ))}
              </div>
            </div>
...
{/* ── KATILIM MENU ── */}
        ...
        {screen==="ticariMenu"&&(
          <div style={{padding:"0 16px 32px"}}>
            ...
            {menuItem("esnekOdemePlanlari","📊","Esnek Ödeme Planları","6 farklı ödeme planı hesaplama",C.green,C.greenLight)}
        ...
        {screen==="esitAnapara"&&<EsitAnapara/>}
        {screen==="araOdemeli"&&<AraOdemeli/>}
        {screen==="artanOdemeli"&&<ArtanOdemeli/>}
        {screen==="azalanOdemeli"&&<AzalanOdemeli/>}
        {screen==="balonOdemeli"&&<BalonOdemeli/>}
        {screen==="esnekOdemeli"&&<EsnekOdemeli/>}
