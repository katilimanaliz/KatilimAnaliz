  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      <Card>
        ...
        <Field label="Kâr Payı Oranı (Yıllık)" value={oran} onChange={setOran} suffix="%" suffix="%"/>
      </Card>
      {r&&<Card>
...
function RaporModal({baslik, satirlar, plan, onClose, showKdv=false, bsmvOran=0, kkdfOran=0}){
  ...
  const metin = [
    ...
    plan.filter(r=>r&&!r._toplamSabitTaksit).map((r,i)=>{
      const taksit=r.taksit||r.toplam;
      const kp=r.karPayi||0;
      const an=r.anapara||0;
      const kdv=r.kdvTutar||0;
      const kalan=r.bakiye||0;
      return kdv>0
        ? `  ${i+1}. ay — Taksit: ${fmt(taksit)} — Kâr Payı: ${fmt(kp)} — Anapara: ${fmt(an)} ${showKdv?"— KDV: "+fmt(kdv)+" ":""}— Kalan: ${fmt(kalan)}`
        : `  ${i+1}. ay — Taksit: ${fmt(taksit)} — Kâr Payı: ${fmt(kp)} — Anapara: ${fmt(an)} — Kalan: ${fmt(kalan)}`;
    }).join("\n") : "",
    "---",
        ...
        <div style={{maxWidth:480,margin:"16px auto",background:"#fff",borderRadius:16,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,0.12)"}}>
          ...
          {plan && plan.length>0 && (
            <div style={{borderTop:"2px solid #1C3A5E"}}>
              ...
              <div style={{minWidth:showKdv?"420px":"520px"}}>
              <div style={{display:"grid",gridTemplateColumns:showKdv?"30px 1fr 1fr 1fr 1fr 1fr":"30px 1fr 1fr 1fr 1fr 1fr 1fr",background:"#EBF3FB",padding:"5px 4px",borderBottom:"1px solid #D1E0EF"}}>
                {(showKdv?["Ay","Taksit","Kâr Payı","Anapara","KDV","Kalan"]:["Ay","Taksit","Kâr Payı","Anapara","BSMV","KKDF","Kalan"]).map((h,i)=>(
                  <span key={i} style={{fontSize:8,fontWeight:700,color:"#1C3A5E",textAlign:i>0?"right":"center",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{h}</span>
                ))}
              </div>
              {/* Tablo satırları */}
              <div style={{maxHeight:320,overflowY:"auto"}}>
                {plan.map((r,i)=>(
                  <div key={i} style={{display:"grid",gridTemplateColumns:showKdv?"30px 1fr 1fr 1fr 1fr 1fr":"30px 1fr 1fr 1fr 1fr 1fr 1fr",padding:"5px 4px",background:i%2===0?"#F8FAFB":"#fff",borderBottom:"1px solid #F0F0F0"}}>
                    <span style={{fontSize:9,color:"#6B7280",textAlign:"center",fontWeight:600}}>{r.ay||i+1}</span>
                    {(showKdv?[r.taksit||r.toplam, r.karPayi||r.faiz, r.anapara, r.kdvTutar||r.vergi||0, r.bakiye]:[r.taksit||r.toplam, r.karPayi||r.faiz, r.anapara, (r.karPayi||0)*bsmvOran/100, (r.karPayi||0)*kkdfOran/100, r.bakiye]).map((v,vi)=>(
                      <span key={vi} style={{fontSize:9,color:"#1a1a1a",textAlign:"right",fontFamily:"monospace",whiteSpace:"nowrap"}}>{fmt(v)}</span>
                    ))}
                  </div>
                ))}
              </div>
              {/* Toplamlar */}
              {plan&&plan.length>0&&(()=>{
                const totTaksit=plan.reduce((s,r)=>s+(r.taksit||r.toplam||0),0);
                const totKP=plan.reduce((s,r)=>s+(r.karPayi||r.faiz||0),0);
                const totAna=plan.reduce((s,r)=>s+(r.anapara||0),0);
                const totKDV=plan.reduce((s,r)=>s+(r.kdvTutar||r.vergi||0),0);
                const fmtC=(n)=>n==null||n===0?"—":new Intl.NumberFormat("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2}).format(n);
                return(
                  <div style={{display:"grid",gridTemplateColumns:showKdv?"30px 1fr 1fr 1fr 1fr 1fr":"30px 1fr 1fr 1fr 1fr 1fr 1fr",padding:"5px 4px",background:"#1C3A5E",borderTop:"2px solid #1C3A5E"}}>
                    <span style={{fontSize:9,fontWeight:800,color:"#fff",textAlign:"center"}}>∑</span>
                    {(showKdv?[totTaksit,totKP,totAna,totKDV,"—"]:[totTaksit,totKP,totAna,plan.reduce((s,r)=>s+(r.karPayi||0)*bsmvOran/100,0),plan.reduce((s,r)=>s+(r.karPayi||0)*kkdfOran/100,0),"—"]).map((v,vi)=>(
                      <span key={vi} style={{fontSize:8,fontWeight:800,color:"#fff",textAlign:"right",whiteSpace:"nowrap"}}>{typeof v==="number"?fmtC(v):v}</span>
                    ))}
                  </div>
                );
              })()}
    ...
    <div style={{padding:"0 16px 40px"}}>
      ...
      <Card>
        ...
        <div style={{position:"relative",marginBottom:16}}>
          ...
          <input inputMode="decimal" value={oran} onChange={e=>setOran(e.target.value.replace(/[^0-9.]/g,""))}
            placeholder="0"
        ...
        <div style={{position:"relative",marginBottom:16}}>
          <input inputMode="decimal" value={vade} onChange={e=>setVade(e.target.value.replace(/[^0-9.]/g,""))}
            placeholder="0"
        ...
        <div style={{position:"relative",width:"48%",marginBottom:16}}>
          <span style={{position:"absolute",left:13,top:"50%",transform:"translateY(-50%)",color:C.blue,fontWeight:700,fontSize:14}}>%</span>
          <input inputMode="decimal" value={kdv} onChange={e=>setKdv(e.target.value.replace(/[^0-9.]/g,""))}
            placeholder="0"
            style={{width:"100%",boxSizing:"border-box",padding:"11px 14px 11px 32px",fontSize:16,fontWeight:600,fontFamily:"monospace",background:"#F9F9FB",border:`1.5px solid ${C.border}`,borderRadius:10,color:"#1C1C1E",outline:"none"}}/>
        </div>
        {/* Kullandırım Komisyonu */}
      ...
      <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
        <div style={{background:"#0F1923",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
          ...
          <div style={{padding:"12px 16px",borderTop:"1px solid #2A3A4A",flexShrink:0}}>
            <button onClick={kopyala} style={{width:"100%",padding:"12px",borderRadius:12,border:"none",background:"#2563EB",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>
              📋 Metni Kopyala
            </button>
            <p style={{margin:"6px 0 0",fontSize:10,color:"#64748B",textAlign:"center"}}>WhatsApp, Mail veya başka bir uygulamaya yapıştırabilirsiniz.</p>
          </div>
        </div>
      </div>
    )}
...
function FinansalGostergeler(){
  ...
  const FALLBACK=[
    ...
    {ad:"CHF/TRY",deger:"57,0555",canli:false},
    ...
    const loadGostergeler = async () => {
      ...
      if(d){
        setKurlar([
          ...
          {ad:"Altın/TRY (Gram)", deger:fmt(d.XAU_TRY_gram,2), canli:d.XAU_TRY_gram!=null},
          {ad:"Gümüş/TRY (Gram)",deger:fmt(d.XAG_TRY_gram,2), canli:d.XAG_TRY_gram!=null},
          {ad:"EUR/USD",          deger:fmt(d.EUR_USD,4),       canli:true},
          {ad:"Ons Altın/USD",    deger:fmt(d.XAU_USD,2),      canli:d.XAU_USD!=null},
        ]);
...
// Claude API'ye tool_use ile kur sorgusu yapıyoruz.
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        system: "Sen bir finansal veri asistanısın. Sadece JSON formatında yanıt ver, başka hiçbir şey yazma.",
        messages: [{
          role: "user",
          content: `Şu anki döviz kurlarını bul ve SADECE şu JSON formatında ver (başka hiçbir şey yazma, markdown yok):
...
Sadece JSON, hiçbir açıklama veya ek metin olmadan.`
        }]
      })
    });
    const data = await res.json();
    // tool_use bloklarını atla, text bloğunu al
  ...
  return(
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif",background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto"}}>
        ...
        {screen==="home"?(
          <div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <div>
                <div style={{display:"flex",gap:6,marginBottom:4}}>
                  <div style={{flex:1,background:"rgba(255,193,7,0.2)",border:"1px solid rgba(255,193,7,0.6)",borderRadius:8,padding:"6px 10px"}}>
                    <p style={{margin:0,fontSize:10,fontWeight:700,color:"#FFD60A",textAlign:"center"}}>⚠️ TEST AŞAMASINDADIR</p>
                    <p style={{margin:"2px 0 0",fontSize:9,color:"rgba(255,255,255,0.7)",textAlign:"center"}}>Hata ve önerilerinizi bildiriniz</p>
                  </div>
              ...
              <div className="ticker-inner">
                {[...kurlar,...kurlar].map((k,i)=>(
                  <span key={i} style={{
                    ...
                    <span style={{color:kurYuklendi?"#4ADE80":"rgba(255,255,100,0.7)",fontFamily:"monospace"}}>{typeof k.try==="number"?fmtN(k.try,4):k.try}</span>
                  </span>
                ))}
              </div>
            </div>
      ...
      <div style={{paddingTop:0}}>
        ...
        {/* ── HOME ── */}
        {screen==="home"&&(
          <div style={{background:"#0F1923",height:"calc(100vh - 112px)",padding:"8px 10px",boxSizing:"border-box",display:"flex"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"repeat(5,1fr)",gap:8,flex:1}}>
              {[
                ...
                {key:"ticariMenu",    icon:"🏢", label:"Tüzel\nFinansman Araçları",            count:"6 Araç", g1:"#7A5000",g2:"#B07D2E"},
...
{/* ── KATILIM MENU ── */}
        {screen==="katilimMenu"&&(
          <div style={{padding:"0 16px 32px"}}>
            ...
            {menuItem("tahvilBono","📈","Sukuk Kira Sertifikası Getiri Hesaplama","Bireysel ve kurumsal getiri analizi",C.teal,C.tealLight)}
        ...
        {screen==="finansmanMenu"&&(
          <div style={{padding:"0 16px 32px"}}>
            ...
            {menuItem("toggFinansman","🚗","Togg Finansmanı Hesaplama","Özel LTV ve vade koşulları",C.teal,C.tealLight)}
            {menuItem("arsaIsyeri","🏢","Arsa/İşyeri Finansmanı","Ekspertiz değerli finansman hesaplama",C.orange,C.orangeLight)}
        ...
        {screen==="ticariMenu"&&(
          <div style={{padding:"0 16px 32px"}}>
            ...
            {menuItem("leasing","🚙","Finansal Kiralama","KDV dahil ödeme planı hesaplama",C.teal,C.tealLight)}
