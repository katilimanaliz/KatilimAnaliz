// api/asistan-ai.js
// KatılımPlus — Claude tabanlı bankacılık asistanı proxy'si.
// Frontend'deki Asistan bileşeni artık yerel anahtar kelime eşleştirmesi
// (MODUL_MAP/yerelYanit/findYakin/findModul/KB) yapmıyor — her mesajı, sabit
// bir sistem promptu + iki yüklenen belgenin metni + güncel finansal takvim
// özetiyle birlikte doğrudan Anthropic Messages API'ye iletiyor.
//
// NOT (2026-07): "YÜKLENEN BELGE" içeriği, önceden frontend'de (FiyatlamaPro.tsx)
// KB adlı dizide anahtar-kelime etiketli olarak tutulan, Ticari Müşterilerden
// Alınacak Ürün ve Hizmetlere İlişkin Ücretler Hakkında Tebliğ ve Uygulama
// Talimatı + Zorunlu Karşılıklar Hakkında Tebliğ Uygulama Talimatı özetinden
// birebir taşındı (28 madde). Orijinal PDF'lerin tam metni bundan daha
// ayrıntılı olabilir — gerekirse bu sabiti PDF_BELGELER ile değiştirin/genişletin.
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(500).json({ success: false, error: "ANTHROPIC_API_KEY eksik — Vercel ortam değişkenlerine ekleyin." });
  }
  ...
  try {
    ...
    if (r.status < 200 || r.status >= 300) {
      return res.status(502).json({ success: false, error: `Anthropic API hatası (HTTP ${r.status}): ${text.slice(0, 300)}` });
    }
    const json = JSON.parse(text);
    const cevapMetni = (json.content || [])
      .filter((b) => b.type === "text")
      .map((b) => b.text)
      .join("\n")
      .trim();

    return res.status(200).json({ success: true, text: cevapMetni || "Üzgünüm, bir yanıt üretemedim." });
  } catch (e) {
    clearTimeout(timer);
