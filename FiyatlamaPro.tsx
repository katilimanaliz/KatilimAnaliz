...
function HaftalikPiyasaOzeti(){
  ...
  const yorum=useMemo(()=>{
    ...
    const p=[];
    p.push(`Bu hafta takip edilen ${satirlar.length} enstrümanın ${yukselen} tanesi değer kazandı.`);
    ...
    if(usd&&xu){
      ...
      else p.push(`Dolar kuru (${(usd.getiri>=0?"+":"")}${usd.getiri.toFixed(2).replace(".",",")}%), bu hafta BIST 100'ün (${(xu.getiri>=0?"+":"")}${xu.getiri.toFixed(2).replace(".",",")}%) önünde yer aldı.`);
    }
    if(fonHafta!=null){
      ...
      <div style={{background:"linear-gradient(135deg,#16243A 0%,#0F1923 70%)",border:"1px solid rgba(91,155,216,0.3)",borderRadius:16,padding:"14px 16px",marginBottom:14,display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{minWidth:0}}>
          ...
          <p style={{margin:"2px 0 0",fontSize:11.5,color:"rgba(255,255,255,0.5)"}}>
            {donemBas&&donemSon?`${donemBas} – ${donemSon}`:"Son 5 işlem günü"}
          </p>
        </div>
      </div>
      ...
      {yukleniyor&&satirlar.length===0?(
        ...
        <>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,overflow:"hidden",marginBottom:14}}>
            ...
            {fonHafta!=null&&(
              <div style={{display:"flex",alignItems:"center",padding:"11px 14px",borderTop:"1px solid rgba(255,255,255,0.1)",background:"rgba(91,155,216,0.05)"}}>
                <span style={{flex:1,fontSize:12.5,fontWeight:700,color:"#9FC1EA"}}>Para Piyasası Fonları (Ort.)</span>
          ...
          <p style={{margin:"12px 4px 0",fontSize:10,color:"rgba(255,255,255,0.3)",lineHeight:1.5}}>
            Kaynak: Yahoo Finance (son 5 işlem günü kapanışları) ve TEFAS/Fonoloji. Gram fiyatları ons ve USD/TRY üzerinden hesaplanır. Özet metni tablodaki verilerden otomatik üretilir; yatırım tavsiyesi değildir.
          </p>
        </>
      )}
    </div>
...
function App(){
  ...
  const kolonW=IS_NATIVE?680:(genisEkran?Math.max(940,Math.min(1320,Math.round(ekranW*0.80))):430);
  const icerikOlcek=!IS_NATIVE&&ekranW>=1600?1.26:(genisEkran&&ekranW>=1280?1.16:1);
    ...
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif",background:"#0F1923",minHeight:"100dvh",maxWidth:kolonW,margin:"0 auto",
      ...
      <KategoriRenkContext.Provider value={EKRAN_KATEGORI[screen]||null}>
        ...
        {screen==="araclarMenu"&&(
          <div style={{background:"#0F1923",padding:"12px 12px 0",paddingBottom:108,boxSizing:"border-box",overflowY:"auto"}}>
            {[
              {key:"haftalikOzet", icon:"📰", label:"Haftalık Piyasa Özeti", desc:(()=>{const b=new Date();const p=new Date(b);p.setDate(b.getDate()-((b.getDay()+6)%7));const f=(d:Date)=>d.toLocaleDateString("tr-TR",{day:"numeric",month:"long"});return `${f(p)} – ${f(b)} · tablo ve haftalık yorum`;})(), renk:"#5B9BD8", bg:"rgba(91,155,216,0.15)"},
