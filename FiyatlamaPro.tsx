...
const TR = (s: any): string => s == null ? "" : String(s).toLocaleUpperCase("tr-TR");
...
const EKRAN_KATEGORI: Record<string,string> = {
  ...
  // Piyasa & Veriler
  bistHisseTarayici:"piyasa", fonGetiriIzleme:"piyasa", finansalGostergeler:"piyasa", piyasaHaberleri:"piyasa",
...
function BistHisseTarayici({ initialTicker, onInitialTuketildi }: { initialTicker?: string | null; onInitialTuketildi?: () => void } = {}) {
  ...
  return (
    <div style={{background:C.bg, padding:"12px 14px 80px", minHeight:"100%"}}>
      ...
      {/* Sektör çipleri + Katılım Endeksi — aynı satırda, Sektör'ün hemen yanında */}
      <div className="piyasa-scroll" style={{display:"flex",gap:7,overflowX:"auto",marginBottom:10,paddingBottom:2}}>
        {sektorler.map(s=>(
          <button key={s} onClick={()=>setSektor(s)} style={{
            flex:"0 0 auto",padding:"7px 13px",borderRadius:18,fontSize:11.5,fontWeight:700,whiteSpace:"nowrap",
            border:`1px solid ${sektor===s?C.blue:C.border}`,
            background:sektor===s?C.blueLight:"transparent",
            color:sektor===s?C.blue:C.sub,cursor:"pointer",fontFamily:"inherit",
          }}>
            {s === "Tümü" ? "Sektör" : (SEKTOR_TR[s] || s)}
          </button>
        ))}
        <button
          onClick={() => setSadeceKatilim(k => !k)}
          style={{
            flex:"0 0 auto",padding:"7px 13px",borderRadius:18,border:"none",cursor:"pointer",fontFamily:"inherit",
...
function KatilimEndeksiTopHareketliler({ nav, onSecim }: { nav: (sc: string) => void; onSecim?: (tur: "hisse" | "fon", sembol: string) => void }) {
  ...
  const fonYukselen = useMemo(() => [...fonOK].sort((a, b) => b.gunluk - a.gunluk).slice(0, 5), [fonOK]);
  const fonDusen     = useMemo(() => [...fonOK].sort((a, b) => a.gunluk - b.gunluk).slice(0, 5), [fonOK]);
  ...
  const Row = ({ item, index }: { item: any; index: number }) => {
    ...
    const deg     = isHisse ? item.degisim1g : item.gunluk;
...
function OdemePlani({plan, bsmvOran, kkdfOran, onClose, showKomisyon, basitOran, efektifOran, anaparaTutar, taksitAraligiGun}){
  ...
  const thStyle={padding:"6px 3px",color:"#fff",fontWeight:700,fontSize:8.5,whiteSpace:"nowrap",textAlign:"right",background:"#1C3A5E",letterSpacing:"0.02em",position:"sticky" as const,top:0,zIndex:1};
  const tdStyle=(color)=>({padding:"5px 3px",fontFamily:"monospace",fontSize:9.5,textAlign:"right",color:color||C.label,whiteSpace:"nowrap"});
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
      <div style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"14px 18px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <span style={{fontSize:16,fontWeight:800,color:C.label}}>📅 Ödeme Planı</span>
        ...
        <p style={{margin:"6px 12px 0",fontSize:9,color:C.sub,textAlign:"right",flexShrink:0}}>↔︎ Tabloyu kaydırarak tüm sütunları görüntüleyebilirsiniz</p>
        <div style={{flex:1,overflow:"auto",WebkitOverflowScrolling:"touch",position:"relative"}}>
          <table style={{borderCollapse:"collapse",width:"100%",minWidth:hasTax?480:340}}>
            <thead>
              <tr>
            ...
            <tbody>
              {plan.map((row,i)=>(
                <tr key={i} style={{background:i%2===0?"rgba(255,255,255,0.03)":"transparent",borderBottom:`1px solid ${C.border}`}}>
                  ...
                  <td style={{...tdStyle(C.label),textAlign:"left",fontFamily:"inherit",fontSize:11}}>{row.tarih||getTarih(i)}</td>
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      ...
      {mod==="pesin"&&rPesin&&<Card>
        <SecTitle>Yatırım Fonu Finansmanı — İlk Taksit Peşin</SecTitle>
        ...
        <RaporButon baslik="Yatırım Fonu Finansmanı — İlk Taksit Peşin" plan={rPesin.plan} satirlar={[
          {label:"Anapara", value:fmtTL(parseFloat(tutar)), big:true},
...
function TaksitenKredi({s}){
  ...
  const r = useCallback(()=>{
    ...
    const netTaksit = tk / vergiCarpan; // bilgi amaçlı (anapara+kâr payı kısmı, ilk ay bazında)
      ...
      <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
        <div style={{background:"#0F1923",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"8px 12px 16px"}}>
            <div style={{display:"grid",gridTemplateColumns:"26px 62px 1fr 1fr 1fr 1fr",background:"#1C3A5E",padding:"7px 8px",borderRadius:"8px 8px 0 0"}}>
              {["Ay","Tarih","Taksit","Kâr Payı","Anapara","Kalan"].map((h,i)=>(
                <span key={i} style={{fontSize:9,fontWeight:800,color:"#fff",textAlign:i>1?"right":"center"}}>{h}</span>
              ))}
            </div>
            {plan_rows.map((p,i)=>(
              <div key={i} style={{display:"grid",gridTemplateColumns:"26px 62px 1fr 1fr 1fr 1fr",padding:"6px 8px",background:i%2===0?"#1C2A38":"#162030"}}>
                <span style={{fontSize:9,color:"#64748B",textAlign:"center"}}>{p.ay||i+1}</span>
                <span style={{fontSize:9,color:"#64748B",textAlign:"center"}}>{p.tarih||"—"}</span>
...
const ASISTAN_KISALTMALAR:{[k:string]:string}={
  "zk":"zorunlu karşılık","kkdf":"kaynak kullanımını destekleme fonu","bsmv":"banka ve sigorta muameleleri vergisi",
  "tm":"teminat mektubu","yp":"yabancı para döviz","ltv":"kredi teminat oranı finansman değer oranı",
  "kmh":"kredili mevduat hesabı","söik":"sevk öncesi ihracat finansmanı","soik":"sevk öncesi ihracat finansmanı",
  "ppk":"para politikası kurulu tcmb faiz","fomc":"fed faiz kararı","tlref":"türk lirası gecelik referans oranı",
  "kkm":"kur korumalı mevduat","f/k":"fiyat kazanç oranı","pd/dd":"piyasa değeri defter değeri",
  "roe":"özkaynak kârlılığı","favök":"faiz amortisman vergi öncesi kâr","gyo":"gayrimenkul yatırım ortaklığı",
  "gmyo":"gayrimenkul yatırım ortaklığı","byf":"borsa yatırım fonu","viop":"vadeli işlem ve opsiyon piyasası",
  "kap":"kamuyu aydınlatma platformu","spk":"sermaye piyasası kurulu","bddk":"bankacılık düzenleme ve denetleme kurumu",
  "tcmb":"türkiye cumhuriyet merkez bankası","tmsf":"tasarruf mevduatı sigorta fonu","kgf":"kredi garanti fonu",
  "eft":"elektronik fon transferi","fast":"anlık ödeme sistemi","dbs":"doğrudan borçlandırma sistemi",
  "dask":"zorunlu deprem sigortası","tefas":"türkiye elektronik fon alım satım platformu",
  "iban":"uluslararası banka hesap numarası","swift":"uluslararası bankalar arası mesajlaşma",
};

// Genel bankacılık soru-cevapları: regex → yanıt üreten fonksiyon (settings alır)
const ASISTAN_GENEL_BILGI:[RegExp,(s:any)=>string][]=[
  [/stopaj/i,(s)=>`📊 **Katılım Hesabı Stopaj Oranları** (gün bazlı)\n• 0–180 gün: %${s?.stopajTL_180??15}\n• 181–365 gün: %${s?.stopajTL_181_365??12}\n• 365 gün üzeri: %${s?.stopajTL_365plus??10}\n• YP hesaplar (tüm vadeler): %${s?.stopajYP_tum??25}\n\nStopaj brüt getiriden kaynakta kesilir; net getiri = brüt − stopaj. Oranlar Ayarlar ekranından güncellenebilir.`],
  [/kkdf|kaynak kullan/i,(s)=>`📊 **KKDF (Kaynak Kullanımını Destekleme Fonu)**\nKâr payı üzerinden alınır:\n• Bireysel finansman: %${s?.bireyselKKDF??15}\n• Konut finansmanı: **muaf (0)**\n• Ticari/tüzel: %${s?.ticariKKDF??0} (genellikle muaf)\n• YP finansmanlarda genellikle 0\n\nÖdeme planları KKDF'yi her ayın kâr payından otomatik hesaplar.`],
  [/bsmv|sigorta muamele/i,(s)=>`📊 **BSMV (Banka ve Sigorta Muameleleri Vergisi)**\nKâr payı üzerinden alınır:\n• Bireysel: %${s?.bireyselBSMV??15} — Ticari: %${s?.ticariBSMV??5}\n• Konutta ilk konut istisnası uygulanabilir\n• YP finansmanlarda genellikle alınmaz\n\nTaksitler "brüt oran" yöntemiyle (akdi oran × (1+BSMV+KKDF)) kurulur; taksit sabittir, vergi her ayın azalan kâr payından hesaplanır.`],
  [/gün esas|360 mı|365 mi|kaç gün üzerinden/i,()=>`📐 **Gün Esası Kuralı (bu uygulamada)**\n• Finansman (kredi) hesapları: **360 gün**\n• Katılım hesabı / mevduat getiri: **365 gün**\n\nÖrn. spot finansman kâr payı = Anapara × (yıllık oran/360) × gün.`],
  [/efektif (oran|maliyet)|yıllık maliyet/i,()=>`📐 **Efektif Yıllık Maliyet**\nMasraflar düşülmüş net kullandırım üzerinden, vergi dahil taksitlerle iç verim oranı bulunur ve (1+i)¹²−1 ile yıllıklandırılır. Akdi orandan her zaman yüksektir çünkü vergi ve masrafları içerir. Finansman ekranları bunu otomatik gösterir.`],
  [/eşit taksit|pmt|anüite|annüite/i,()=>`📐 **Eşit Taksit (PMT/Anüite)**\nTaksit = T × i / (1 − (1+i)⁻ⁿ)\nT=anapara, i=aylık brüt oran (akdi × (1+vergi)), n=vade(ay). Her taksitte kâr payı azalır, anapara payı artar.`],
  [/fast.*(limit|saat|nedir)|eft.*(saat|limit|nedir)|havale.*(saat|limit|fark)/i,()=>`🏦 **EFT / FAST / Havale**\n• Havale: aynı banka içi, 7/24\n• EFT: bankalar arası, iş günü ~08:30–17:00\n• FAST: bankalar arası **7/24 anlık**; işlem limiti TCMB tarafından belirlenir ve dönemsel güncellenir — güncel rakam için TCMB duyurusuna bakılmalı, buradan kesin değer veremem.`],
  [/kur korumalı|kkm/i,()=>`🏦 **KKM (Kur Korumalı Mevduat)**\nTL mevduata kur farkı desteği veren üründü; 2024'ten itibaren kademeli sonlandırıldı, yeni hesap açılışı durduruldu. Katılım karşılığı "kur korumalı katılma hesabı" idi.`],
  [/tlref/i,()=>`📊 **TLREF** — TL Gecelik Referans Faiz Oranı. BIST bünyesinde gecelik repo işlemlerinden hesaplanır; değişken getirili ürünlerde referanstır. Güncel değeri Finansal Göstergeler ekranında görebilirsiniz.`],
  [/iban nedir/i,()=>`🏦 **IBAN** — Türkiye'de TR + 24 hane (26 karakter): TR + 2 kontrol + 5 banka kodu + 1 rezerv + 16 hesap no.`],
  [/rotatif/i,()=>`🏦 **Rotatif Finansman** — limit dahilinde esnek kullanım/geri ödeme; kâr payı kullanılan tutar × gün üzerinden işler. Spot'tan farkı: spot tek seferde kullandırılır, vade sonunda kapanır.`],
  [/valör/i,()=>`🏦 **Valör** — işlemin getiri hesabına girmeye başladığı tarih. Yatırılan para genelde ertesi iş günü valör alır, çekilen aynı gün kaybeder.`],
  [/temerrüt/i,()=>`🏦 **Temerrüt** — borcun vadesinde ödenmemesi. Sözleşmedeki temerrüt oranı işletilir; katılım bankalarında gecikme bedeli danışma kurulu kararıyla hayır kurumlarına bağışlanabilir.`],
  [/akreditif nedir/i,()=>`🏦 **Akreditif** — dış ticarette, alıcının bankasının satıcıya şartlı ödeme taahhüdü. Katılım bankacılığında vekâlet/kefalet esaslı yürütülür. Komisyon hesabı için Akreditif Komisyon ekranını kullanabilirsiniz.`],
  [/faktoring|forfaiting/i,()=>`🏦 **Faktoring** — vadeli alacakların temlik yoluyla finansmanı (yurtiçi ağırlıklı, rücu edilebilir/edilemez). **Forfaiting** — genellikle yurtdışı, orta-uzun vadeli alacakların rücusuz iskontosu.`],
];

// Takvim zekâsı: "sonraki PPK ne zaman" gibi sorulara gerçek tarihle yanıt
function asistanTakvimYaniti(q:string):string|null{
  const n=trNorm(q);
  const TIP_ESLE:[RegExp,string,string][]=[
    [/ppk|para politikası/, "ppk", "PPK Toplantısı"],
    [/fomc|fed/, "fed", "FED (FOMC) Faiz Kararı"],
    [/zorunlu karşılık|\bzk\b/, "zk", "ZK Hesaplama"],
    [/tl payı/, "tlpayi", "TL Payı Rasyo Hesaplama"],
    [/kredi büyüme/, "kredi", "Kredi Büyüme Hesaplama"],
  ];
  const zamanSoruyor=/ne zaman|sonraki|bir sonraki|tarih|hangi gün|kaç gün/.test(n);
  if(!zamanSoruyor) return null;
  for(const [re,tip,ad] of TIP_ESLE){
    if(re.test(n)){
      const bugun=new Date(); bugun.setHours(0,0,0,0);
      const ev=katilimFinansalTakvimEvents().filter((e:any)=>e.tip===tip&&e.tarih>=bugun).sort((a:any,b:any)=>a.tarih-b.tarih)[0];
      if(!ev) return `📅 ${ad} için takvimde ileri tarihli kayıt bulamadım.`;
      const gunFark=Math.round((ev.tarih.getTime()-bugun.getTime())/86400000);
      const ts=ev.tarih.toLocaleDateString("tr-TR",{day:"numeric",month:"long",year:"numeric",weekday:"long"});
      return `📅 **Bir sonraki ${ad}:** ${ts}${gunFark===0?" — **bugün!**":` (${gunFark} gün sonra)`}\n\nTüm tarihleri Finansal Takvim ekranında görebilirsiniz.`;
    }
  }
  return null;
}

// Hesaplama niyeti: sayı veya hesap fiili geçiyorsa modül CTA'sı güçlendirilir
function asistanHesapNiyeti(q:string){
  const n=trNorm(q);
  return /\d/.test(n) || /hesapla|taksit|kaç olur|ne kadar|maliyet çıkar|ödeme plan/.test(n);
}
...
function Asistan({nav, settings}:{nav:any, settings?:any}){
  ...
  // ── Modül yönlendirme ─────────────────────────────────────────────────────
  const MODUL_MAP=[
    {keys:["konut","mortgage","ev finansman","ev kredisi","ev almak"],screen:"konutFinansman",label:"Konut Finansmanı"},
    ...
    {keys:["leasing","finansal kiralama","icara"],screen:"leasing",label:"Finansal Kiralama"},
    {keys:["spot"],screen:"spotFinansman",label:"Spot Finansman"},
    {keys:["rotatif","taksitli ticari","ticari finansman","tl büyüme","yp büyüme","tl muaf","yp muaf","tl istisna","yp istisna","büyüme sınır","büyüme istisna"],screen:"taksitliTicari",label:"Taksitli Ticari Finansman"},
    {keys:["pos","üye işyeri"],screen:"posHesaplama",label:"POS Komisyon Analizi"},
    {keys:["sukuk","kira sertifikası","tahvil","bono"],screen:"tahvilBono",label:"Sukuk Kira Sertifikası"},
    {keys:["katılım hesabı","vadeli hesap","mevduat getiri","param ne kadar getirir","vadeli getiri"],screen:"vadeliKatilim",label:"Katılım Hesabı Getiri"},
    ...
    {keys:["yatırım fonu finansman"],screen:"yatirimFonuFinansman",label:"Yatırım Fonu Finansmanı"},
    {keys:["arsa","işyeri"],screen:"arsaIsyeri",label:"Arsa/İşyeri Finansmanı"},
    {keys:["zorunlu karşılık","verimlilik"],screen:"verimlilikAnalizi",label:"Verimlilik Analizi"},
    {keys:["katkı payı"],screen:"katkiPayi",label:"Katkı Payı Hesaplama"},
    {keys:["taksitten anapara","taksitten kredi","taksite göre"],screen:"taksitenKredi",label:"Taksitten Anapara"},
    {keys:["getiriden anapara","şu kadar kazanmak için"],screen:"getiridenAnapara",label:"Getiriden Anapara"},
    {keys:["günlük hesap","günlük oran"],screen:"oranAnalizi",label:"Günlük Hesap Oran"},
    {keys:["kasa hesabı","kasa oran"],screen:"kasaOranAnalizi",label:"Basitten Bileşiğe Oran Hesaplama"},
    {keys:["sevk öncesi ihracat","reeskont"],screen:"soikReeskont",label:"SÖİK ve Reeskont Finansmanı Hesaplama"},
    {keys:["döviz çevir","kur çevir","döviz dönüştür","dolar kaç tl","euro kaç tl"],screen:"hazineDoviz",label:"Döviz Dönüştürücü"},
    {keys:["forward"],screen:"hazineForward",label:"Forward Hesaplama"},
    {keys:["swap"],screen:"hazineSwap",label:"Swap Hesaplama"},
    {keys:["hazine bonosu"],screen:"hazineBono",label:"Hazine Bonosu Getiri"},
    {keys:["kur senaryo","kur hareket"],screen:"hazineSenaryo",label:"Kur Senaryo Analizi"},
    {keys:["esnek ödeme","ara ödeme","balon ödeme","artan ödeme","azalan ödeme","eşit anapara"],screen:"esnekOdemePlanlari",label:"Esnek Ödeme Planları"},
    {keys:["hisse","bist"],screen:"bistHisseTarayici",label:"BİST Hisse Veri İzleme"},
    {keys:["fon getiri","fon takip","fonlar"],screen:"fonGetiriIzleme",label:"Yatırım Fonları Getiri İzleme"},
    {keys:["haber"],screen:"piyasaHaberleri",label:"Piyasa Haberleri"},
    {keys:["takvim","ekonomik takvim"],screen:"finansalTakvim",label:"Finansal Takvim"},
    {keys:["gösterge","tüfe","enflasyon","politika faizi"],screen:"piyasaMenu",label:"Piyasa & Veriler (Göstergeler)"},
    {keys:["vade takip","hatırlatma"],screen:"vadeTakibi",label:"Vade Takip & Hatırlatma"},
    {keys:["sözlük","terim"],screen:"sozluk",label:"Katılım Bankacılığı Sözlüğü"},
  ];
  ...
  const SOZLUK_KISA={
    ...
    "selem":"Bedeli peşin ödenen, malı ileride teslim edilen satış sözleşmesi. Tarım finansmanında kullanılır.",
    "istisna":"Sipariş üzerine üretim sözleşmesi; bedel taksitle ödenebilir. Proje/konut finansmanında kullanılır.",
    "teverruk":"Nakit ihtiyacı için: vadeli mal alıp peşin satma yöntemi. Organize teverruk katılım bankalarında likidite yönetiminde kullanılır.",
    "wakala":"Vekâlet esaslı yatırım: banka, müşteri fonunu belirlenen beklenen getiriyle yönetir.",
    "danışma kurulu":"Katılım bankasının ürün ve işlemlerinin faizsizlik ilkelerine uygunluğunu denetleyen kurul.",
    "katılma hesabı":"Kâr/zarara katılma esaslı hesap; getiri garanti edilmez, birim değer üzerinden hesaplanır.",
    "özel cari hesap":"Vadesiz, getirisiz; istenildiğinde çekilebilen katılım bankası hesabı.",
    "birim değer":"Katılma hesabı havuzunun günlük değerini gösteren katsayı; getiri birim değer artışından hesaplanır.",
    "iştira":"Vadeli alacağın (genelde senet) vadesinden önce iskonto edilerek devralınması.",
    "kâr payı":"Katılım bankacılığında faiz yerine kullanılan, ticaret/ortaklık kaynaklı getiri.",
  };
...
📐 Detaylı hesaplama için ilgili finansman modülünü kullanın.`;
  ...
  const yerelYanit=(q)=>{
    const ql=trNorm(q);
    // Kısaltmaları açarak genişletilmiş sorgu (arama bunun üzerinden yapılır)
    let genisQ=ql;
    const bulunanKisaltmalar:string[]=[];
    for(const [kisa,acilim] of Object.entries(ASISTAN_KISALTMALAR)){
      if(new RegExp(`(^|[^a-zçğıöşü])${kisa.replace("/","\\/")}([^a-zçğıöşü]|$)`,"i").test(ql)){
        genisQ+=" "+acilim;
        bulunanKisaltmalar.push(kisa.toUpperCase());
      }
    }

    // 0. Takvim zekâsı ("sonraki PPK ne zaman" vb.)
    const takvim=asistanTakvimYaniti(genisQ);
    if(takvim) return {text:takvim,tip:"takvim",bulunanKisaltmalar};

    // 1. Sözlük araması
    for(const [terim,tanim] of Object.entries(SOZLUK_KISA)){
      if(genisQ.includes(terim)){
        return {text:`📖 **${terim.charAt(0).toUpperCase()+terim.slice(1)}**

${tanim}`,tip:"sozluk",bulunanKisaltmalar};
      }
    }

    // 2. Genel bankacılık bilgisi (stopaj, KKDF, BSMV, EFT/FAST, TLREF...)
    for(const [re,uret] of ASISTAN_GENEL_BILGI){
      if(re.test(genisQ)) return {text:uret(settings),tip:"genel",bulunanKisaltmalar};
    }

    // 3. KB araması (ZK Talimatı & Ücretler Tebliği)
    const found=findAnswer(genisQ);
    if(found){
      return {text:`📋 **${found.title}**

${found.content}`,tip:"kb",bulunanKisaltmalar};
    }

    // 4. Finansal analiz
    const analiz=analizYap(genisQ);
    if(analiz) return {text:analiz,tip:"analiz",bulunanKisaltmalar};

    // 5. Bulunamadı
    return {text:`"${q.slice(0,50)}" sorusuna doğrudan bir yanıt bulamadım.

Yardımcı olabileceğim konular:
• ZK oranları, stopaj dilimleri, KKDF/BSMV
• Katılım bankacılığı terimleri (murabaha, sukuk, icara...)
• Genel bankacılık (EFT/FAST, TLREF, valör, rotatif, temerrüt...)
• "Sonraki PPK/FED/ZK günü ne zaman?" gibi takvim soruları
• Kısaltmalar: ZK, TM, LTV, KKDF, BSMV, KKM, F/K, ROE...
• Herhangi bir hesaplama — ilgili ekrana yönlendirebilirim

💼 Konuyla ilgili kesin bilgi için bölge veya segment temsilcinizle iletişime geçebilirsiniz.`,tip:"bulunamadi",bulunanKisaltmalar};
  };
  ...
  const send=async()=>{
    ...
    const {text,tip,bulunanKisaltmalar}=yerelYanit(q);
    // Modül eşleşmesi de kısaltma açılımlı sorgu üzerinden yapılır
    let genisQ=trNorm(q);
    for(const [kisa,acilim] of Object.entries(ASISTAN_KISALTMALAR)){
      if(new RegExp(`(^|[^a-zçğıöşü])${kisa.replace("/","\\/")}([^a-zçğıöşü]|$)`,"i").test(genisQ)) genisQ+=" "+acilim;
    }
    // Takvim tipi sorularda (ör. "ZK hesaplama tarihleri") kısaltma genişletmesi
    // yüzünden alakasız bir hesap modülü (ör. Verimlilik Analizi) önerilmesin —
    // zaten takvim yanıtı verildiği için doğrudan Finansal Takvim'e yönlendir.
    const modul = tip==="takvim" ? {screen:"finansalTakvim",label:"Finansal Takvim"} : findModul(genisQ);
    const hesapIstiyor=asistanHesapNiyeti(q);

    const ekler=[];
    if(yakin) ekler.push({tip:"oneri",text:`💡 "${yakin}" terimini mi sormak istediniz?`});
    if(modul) ekler.push({tip:"modul",
      text: tip==="takvim"
        ? `📅 Tüm tarihleri gör: ${modul.label}`
        : hesapIstiyor
        ? `🧮 Bu hesaplamayı yapmak için: ${modul.label} → Ekrana Git`
        : `📐 ${modul.label} → Hesaplamaya Git`,
      screen:modul.screen,label:modul.label});
    if(tip==="bulunamadi"){
      if(!modul) ekler.push({tip:"uyari",text:"⚠️ Bu konu yerel bilgi tabanımda yer almıyor."});
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 14px"}}>
          ...
          <div style={{
            ...
            <p style={{margin:"12px 0 0",fontSize:10,color:dinliyor?"#F87171":"rgba(255,255,255,0.3)",textAlign:"center"}}>
              {dinliyor?"🔴 Dinleniyor… durdurmak için mikrofona tekrar dokun":"📄 ZK Talimatı & Ücretler Tebliği + genel bankacılık bilgisi · kısaltmaları anlar · hesaplamaya yönlendirir"}
            </p>
          </div>
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      ...
      {r&&r.brutFaiz!==undefined&&<Card>
        ...
        <div style={{height:1,background:C.border,margin:"6px 0"}}/>
        <RRow label={`Gelir (${r.vadeGun} Gün)`} value={fmtDovizVA(r.brutFaiz)} accent={C.orange} big/>
  ...
  return(
    <div style={{paddingBottom:32,background:"#0F1923",minHeight:"100dvh"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#1A3A2A 0%,#2A5A3A 100%)",padding:"16px 16px 14px"}}>
        <p style={{margin:"0 0 2px",fontSize:11,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:"0.07em"}}>{TR("Katılım Analiz")}</p>
...
// ─── FİNANSAL TAKVİM ────────────────────────────────────────────────────────
// Katılım Analiz'e özel finansal takvim etkinlikleri (ZK, PPK, FED, TL Payı, Kredi Büyüme) — ortak fonksiyon
...
const KATILIM_BANKALARI = [
  {
    ad:"Albaraka Türk Katılım Bankası", kisa:"Albaraka Türk", kurulus:"1985", renk:"#00A19A", harf:"A",
  ...
  {
    ad:"Kuveyt Türk Katılım Bankası", kisa:"Kuveyt Türk", kurulus:"1989", renk:"#00954C", harf:"K",
  ...
  {
    ad:"Türkiye Finans Katılım Bankası", kisa:"Türkiye Finans", kurulus:"2005", renk:"#E30613", harf:"T",
  ...
  {
    ad:"Ziraat Katılım Bankası", kisa:"Ziraat Katılım", kurulus:"2015", renk:"#00693E", harf:"Z",
  ...
  {
    ad:"Vakıf Katılım Bankası", kisa:"Vakıf Katılım", kurulus:"2015", renk:"#1A4F9C", harf:"V",
  ...
  },
  {
    ...
    ozet:"Ahlatcı Holding bünyesinde faaliyete geçen en yeni katılım bankasıdır. Katılım prensipleriyle yatırım ve ticari bankacılık çözümleri sunar.",
    detaylar:[
      "2024'te faaliyete geçen sektörün en yeni oyuncusudur.",
      "Ahlatcı Holding bünyesinde kuruldu; TKBB üye sayısını 9'a yükseltti.",
    ...
    return(
      <div style={{background:"#0F1923",padding:"12px 14px 92px",minHeight:"100%"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
          <div style={{width:60,height:60,borderRadius:16,background:secili.renk,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,fontWeight:800,color:"#fff",flexShrink:0}}>{secili.harf}</div>
          <div style={{flex:1,minWidth:0}}>
        ...
        <p style={{margin:"16px 0 0",fontSize:10,color:"rgba(255,255,255,0.3)",textAlign:"center",lineHeight:1.5}}>Bilgiler kamuya açık kaynaklardan (TKBB/BDDK) derlenmiştir · Ocak–Mart 2026</p>
      </div>
    );
  }
    ...
    <div style={{background:"#0F1923",padding:"12px 14px 92px",minHeight:"100%"}}>
      ...
      {KATILIM_BANKALARI.map((b,i)=>(
        <div className="press-card" key={i} onClick={()=>setSecili(b)} style={{
          ...
          <div style={{width:46,height:46,borderRadius:12,background:b.renk,display:"flex",alignItems:"center",justifyContent:"center",fontSize:21,fontWeight:800,color:"#fff",flexShrink:0}}>{b.harf}</div>
          <div style={{flex:1,minWidth:0}}>
...
function GostergeGrafikModal({ad,seri,birim,onClose}:{ad:string,seri:{tarih:string,deger:number}[],birim?:string,onClose:()=>void}){
  ...
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:600,display:"flex",alignItems:"flex-end"}} onClick={onClose}>
      <div style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"75vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
...
const TAB_OF_SCREEN:any = {
  ...
  araclarMenu:"araclar", sozluk:"araclar", vadeTakibi:"araclar", katilimBankalari:"araclar",
  asistan:"yapayzeka",
  profil:"profil",
};
...
const MODUL_TO_KEY: Record<string,string> = {
  ...
  "Yatırım Fonu Finansmanı":"yatirimFonuFinansman",
      ...
      <div style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 18px 32px"}}>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px"}}>
            ...
            <p style={{margin:0,fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.5}}>Bu uygulamadaki hesaplamalar bilgilendirme amaçlıdır; kesin teklif, resmi belge veya hukuki taahhüt niteliği taşımaz ve hukuki sonuç doğurmaz. Nihai oranlar ve koşullar için yetkili biriminizle iletişime geçiniz.</p>
          </div>
...
function PiyasaOzetiKart({ad,sembol,paraOnek,dec,onTikla}:{ad:string,sembol:string,paraOnek?:string,dec:number,onTikla:()=>void}){
  ...
  const kucukIkon = (()=>{
    ...
    if(a.includes("USD")||a.includes("DOLAR")) return {tip:"emoji",deger:"💵"};
    if(a.includes("EUR")) return {tip:"emoji",deger:"🇪🇺"};
    if(a.includes("GBP")||a.includes("STERLİN")) return {tip:"emoji",deger:"🇬🇧"};
    if(a.includes("ALTIN")||a.includes("ALTİN")) return {tip:"emoji",deger:"🥇"};
    if(a.includes("GÜMÜŞ")||a.includes("GUMUS")) return {tip:"emoji",deger:"🥈"};
  ...
  return(
    <div className="press-card" onClick={onTikla} style={{
      ...
      {kucukIkon&&(kucukIkon.tip==="rozet"?(
        <span style={{position:"absolute",top:7,right:8,width:16,height:16,borderRadius:8,background:kucukIkon.bg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,color:"#fff",lineHeight:1}}>{kucukIkon.deger}</span>
      ):(
        <span style={{position:"absolute",top:8,right:9,fontSize:12,opacity:0.85,lineHeight:1}}>{kucukIkon.deger}</span>
      ))}
      <p style={{margin:0,fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:0.2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",paddingRight:16}}>{TR(ad)}</p>
      {guncel!=null ? (
        <>
...
function App(){
  ...
  // Ana sayfa "Yaklaşan Takvim" — önümüzdeki 3 gün, Türkiye ekonomik takvimi + katılım bankacılığı takvimi (ZK/PPK/FED/TL Payı/Kredi Büyüme)
  const yaklasanTakvim = useMemo(() => {
    const bugun = new Date(); bugun.setHours(0,0,0,0);
    const bitis = new Date(bugun); bitis.setDate(bitis.getDate()+3); bitis.setHours(23,59,59,999);
          ...
          <div>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10,marginBottom:4}}>
              <div style={{display:"flex",alignItems:"center",gap:7,flexShrink:0}}>
                <div style={{width:30,height:30,borderRadius:15,border:"1.5px solid #5B9BD8",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,background:"rgba(91,155,216,0.08)"}}>
                  <img src={KATILIM_LOGO_B64} alt="" style={{height:15,width:"auto",display:"block"}}/>
                </div>
                <div style={{display:"flex",flexDirection:"column"}}>
                  <span style={{fontSize:14,fontWeight:700,letterSpacing:"-0.005em",fontFamily:"Georgia,'Iowan Old Style','Times New Roman',serif",background:"linear-gradient(100deg,#EAF1FA 0%,#C9D9EE 60%,#B8C9E8 100%)",WebkitBackgroundClip:"text",backgroundClip:"text",color:"transparent"} as any}>Katılım Analiz</span>
                  <span style={{fontSize:9,fontWeight:600,color:"rgba(255,255,255,0.42)",letterSpacing:"0.01em",marginTop:1}}>Katılım Finansının Akıllı Asistanı</span>
                </div>
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:5,flexShrink:0,maxWidth:150,textAlign:"right"}}>
                <div onClick={()=>setBildirimAcik(true)} style={{cursor:"pointer"}}>
                  <p style={{margin:0,fontSize:9,fontWeight:700,color:"#FFD60A",lineHeight:1.3}}>⚠️ TEST AŞAMASINDADIR</p>
                  <p style={{margin:"1px 0 0",fontSize:7.5,color:"rgba(255,255,255,0.55)",lineHeight:1.3}}>Hata ve önerilerinizi buradan bildirebilirsiniz</p>
                </div>
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
              <span style={{width:3,height:14,borderRadius:2,background:"#5B9BD8",flexShrink:0}}/>
              <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:1}}>{TR(selamlama)} · {kisaTarihStr}</span>
            </div>
            {(()=>{
              const selamMetni = kullaniciAdi?`Hoş geldin ${kullaniciAdi}`:"Hoş geldin";
              // Uzun isim/soyisim girilirse başlık 2 satıra taşıp bölünmesin diye
              // metin uzunluğuna göre yazı boyutu otomatik küçültülüyor (32px tabanlı,
              // 18px'e kadar), tek satırda kalması garanti ediliyor.
              const uzunluk = selamMetni.length;
              const dinamikBoyut = uzunluk<=16 ? 32 : Math.max(18, Math.round(32 - (uzunluk-16)*0.85));
              return (
                <div style={{
                  fontSize:dinamikBoyut,fontWeight:700,fontStyle:"italic",letterSpacing:"-0.01em",marginBottom:6,
            ...
            <div style={{fontSize:13.5,color:"rgba(255,255,255,0.5)",marginBottom:16}}>Bugün ne hesaplamak istersin?</div>
      ...
      <KategoriRenkContext.Provider value={EKRAN_KATEGORI[screen]||null}>
        ...
        {screen==="home"&&(
          <div style={{background:"#0F1923",padding:"10px 12px 0",paddingBottom:108,boxSizing:"border-box",display:"flex",flexDirection:"column",overflowY:"auto"}}>
            ...
            <div className="piyasa-scroll" style={{display:"flex",overflowX:"auto",gap:6,marginBottom:14,scrollSnapType:"x mandatory",WebkitOverflowScrolling:"touch",paddingBottom:2}}>
              {[
                {ad:"USD/TRY",  sembol:"USDTRY=X",   dec:2, kod:"USD",              birim:"₺"},
                {ad:"EUR/TRY",  sembol:"EURTRY=X",   dec:2, kod:"EUR",              birim:"₺"},
                {ad:"GBP/TRY",  sembol:"GBPTRY=X",   dec:2, kod:"GBP",              birim:"₺"},
                {ad:"ALTIN/GR", sembol:"GRAM_ALTIN", dec:2, kod:"Altın/TRY (Gram)", birim:"₺"},
                {ad:"BIST 100", sembol:"XU100.IS",   dec:0, kod:"BIST 100",         birim:"₺"},
                {ad:"BITCOIN",  sembol:"BTC-USD",    dec:0, kod:"Bitcoin",          birim:"$"},
                {ad:"ETHEREUM", sembol:"ETH-USD",    dec:2, kod:"Ethereum",         birim:"$"},
                {ad:"GÜMÜŞ GRAM", sembol:"GRAM_GUMUS", dec:2, kod:"Gümüş/TRY (Gram)",  birim:"₺"},
                {ad:"BRENT",    sembol:"BZ=F",       dec:2, kod:"Brent Petrol",     birim:"$"},
                {ad:"S&P 500",  sembol:"^GSPC",      dec:2, kod:"S&P 500",          birim:"$"},
              ].map(k=>(
                <div key={k.sembol} style={{flex:"0 0 108px",scrollSnapAlign:"start"}}>
                  <PiyasaOzetiKart ad={k.ad} sembol={k.sembol} dec={k.dec} onTikla={()=>setSeciliKur({kod:k.kod,ad:k.ad,sembol:k.sembol,birim:k.birim})}/>
                </div>
              ))}
            </div>
            ...
            <div className="piyasa-scroll" style={{display:"flex",overflowX:"auto",gap:8,marginBottom:14,scrollSnapType:"x mandatory",WebkitOverflowScrolling:"touch",paddingBottom:2}}>
              {favoriler.map(key=>{
                ...
                return(
                  <div className="press-tile" key={key} onClick={()=>nav(item.key)} style={{
            ...
            <div className="press-tile" onClick={()=>nav("asistan")} style={{
              position:"relative",overflow:"hidden",cursor:"pointer",marginBottom:14,
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:18,marginBottom:8}}>
              ...
              <span onClick={()=>{setPiyasaTabloFiltre("gostergeler");nav("piyasaMenu");}} style={{fontSize:11,fontWeight:700,color:"#3B82F6",cursor:"pointer"}}>Tümü ›</span>
            </div>
            <div onClick={()=>{setPiyasaTabloFiltre("gostergeler");nav("piyasaMenu");}} style={{
              ...
              {[
                ...
                {ad:"TÜFE (Yıllık)", deger:evdsMakro?.["TUFE_YILLIK"]?.deger!=null?`%${evdsMakro["TUFE_YILLIK"].deger.toFixed(2).replace(".",",")}`:"—", tarih:evdsMakro?.["TUFE_YILLIK"]?.tarih?`${evdsMakro["TUFE_YILLIK"].tarih} · canlı`:"", ikon:TrendingUp, renk:"#F87171"},
                {ad:"TÜFE (Aylık)", deger:evdsMakro?.["TUFE_AYLIK"]?.deger!=null?`%${evdsMakro["TUFE_AYLIK"].deger.toFixed(2).replace(".",",")}`:"—", tarih:evdsMakro?.["TUFE_AYLIK"]?.tarih?`${evdsMakro["TUFE_AYLIK"].tarih} · canlı`:"", ikon:Activity, renk:"#F87171"},
                {ad:"TLREF (Gecelik Referans)", deger:evdsMakro?.["TP.BISTTLREF.KAPANIS"]?.deger!=null?`%${evdsMakro["TP.BISTTLREF.KAPANIS"].deger.toFixed(2).replace(".",",")}`:"—", tarih:evdsMakro?.["TP.BISTTLREF.KAPANIS"]?.tarih?`${evdsMakro["TP.BISTTLREF.KAPANIS"].tarih} · canlı`:"", ikon:Percent, renk:"#8B5CF6"},
                (()=>{const tlrefk=tlrefkTahmini(evdsMakro); return {
                  ad:"TLREFK (Katılım)",
                  deger:tlrefk?.deger!=null?`%${tlrefk.deger.toFixed(2).replace(".",",")}`:"—",
                ...
                {ad:"TCMB Brüt Rezerv", deger:evdsMakro?.["TP.AB.TOPLAM"]?.deger!=null?`$${(evdsMakro["TP.AB.TOPLAM"].deger/1000).toFixed(2).replace(".",",")} Mr`:"—", tarih:evdsMakro?.["TP.AB.TOPLAM"]?.tarih?`${evdsMakro["TP.AB.TOPLAM"].tarih} · canlı`:"", ikon:Wallet, renk:"#4ADE80"},
              ].map((g,i,arr)=>{
                const IkonBileseni=g.ikon;
                return (
                <div key={i} style={{display:"flex",alignItems:"center",gap:11,padding:"12px 16px",borderBottom:i<arr.length-1?"1px solid rgba(255,255,255,0.06)":"none"}}>
            ...
            {sonHaberler.length>0&&(
              <>
                ...
                <div className="piyasa-scroll" style={{marginBottom:14,maxHeight:246,overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
                  {sonHaberler.map((h,i)=>{
            ...
            {/* Yaklaşan Takvim — önümüzdeki 3 gün, Türkiye */}
            {yaklasanTakvim.length>0&&(
              <>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.5}}>{TR("Yaklaşan Takvim · 3 Gün")}</span>
                  <span onClick={()=>nav("finansalTakvim")} style={{fontSize:11,fontWeight:700,color:"#3B82F6",cursor:"pointer"}}>Tümü ›</span>
                </div>
                <div style={{marginBottom:14}}>
            ...
            {gecmis.length>0 && hesaplaAramaQ==="" && (hesaplaFiltre==="tumu") && (
              <div style={{marginBottom:14}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  ...
                  <span onClick={()=>setHesaplaFiltre("gecmis")} style={{fontSize:11,fontWeight:700,color:"#3B82F6",cursor:"pointer"}}>Tümü ›</span>
                </div>
                <div className="piyasa-scroll" style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:2}}>
        ...
        {screen==="piyasaMenu"&&(()=>{
          ...
          const kaynakVeri = piyasaTabloFiltre==="tumu"
            ? [...(PIYASA_TABLO_VERISI["doviz"]||[]),...(PIYASA_TABLO_VERISI["emtia"]||[]),...(PIYASA_TABLO_VERISI["borsa"]||[]),...(PIYASA_TABLO_VERISI["kripto"]||[])]
        ...
        {screen==="profil"&&(
          <div style={{background:"#0F1923",padding:"12px 12px 0",paddingBottom:108,boxSizing:"border-box",overflowY:"auto"}}>
            ...
            <div style={{
              ...
              />
              <p style={{margin:0,fontSize:11,color:"rgba(255,255,255,0.35)"}}>Sadece bu cihazda saklanır</p>
              ...
              <div style={{display:"flex",marginTop:16,borderTop:"1px solid rgba(255,255,255,0.08)",paddingTop:14}}>
                <div style={{flex:1}}>
                  ...
                  <p style={{margin:"2px 0 0",fontSize:10.5,color:"rgba(255,255,255,0.45)"}}>Favori</p>
                </div>
                <div style={{width:1,background:"rgba(255,255,255,0.08)"}}/>
                <div style={{flex:1}}>
                  <p style={{margin:0,fontSize:20,fontWeight:800,color:"#4ADE80"}}>{gecmis.length}</p>
                  <p style={{margin:"2px 0 0",fontSize:10.5,color:"rgba(255,255,255,0.45)"}}>Hesaplama</p>
                </div>
              </div>
            </div>
            ...
            {[
              ...
              {ikon:FileText, label:"Ana Ekrana Ekleme Kılavuzu", onClick:()=>setKurulumKilavuzuAcik(true)},
              {ikon:Search, label:"Hakkında", onClick:()=>setHakkindaAcik(true)},
              {ikon:Bell, label:"Hata & Öneri Bildir", onClick:()=>setBildirimAcik(true)},
            ].map((r,i)=>(
              ...
              return (
                <div key={t.tab} onClick={()=>nav(t.key)} style={{
                  ...
                  <div style={{
                    ...
                    padding:"2px 8px 5px",maxWidth:"100%",boxSizing:"border-box",
                  }}>
                    <AltBarIcon tip={t.tip} aktif={aktif}/>
                    <span style={{fontSize:10,fontWeight:aktif?700:500,letterSpacing:"0.01em",color:aktif?"#5B9BD8":"rgba(255,255,255,0.45)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"100%",transition:"color 0.2s ease"}}>{t.label}</span>
