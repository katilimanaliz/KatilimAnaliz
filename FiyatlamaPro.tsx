...
const EKRAN_KATEGORI: Record<string,string> = {
  ...
  // Piyasa & Veriler
  bistHisseTarayici:"piyasa", fonGetiriIzleme:"piyasa", karPayiOranlari:"piyasa", finansalGostergeler:"piyasa", piyasaHaberleri:"piyasa",
...
function KarPayiOranlari({nav}:{nav:any}){
  ...
  const enYuksek=(alan:string)=>{
    const degerler=bankalar.filter((b:any)=>typeof b[alan]==="number").map((b:any)=>b[alan]);
    return degerler.length?Math.max(...degerler):null;
  };
  const maxTl=enYuksek("tl"), maxUsd=enYuksek("usd"), maxEur=enYuksek("eur"), maxAltin=enYuksek("altin");
      ...
      {!yukleniyor&&!hata&&!bankalar.every((b:any)=>b.tl==null&&b.usd==null&&b.eur==null&&b.altin==null)&&(
        <div style={{display:"flex",gap:6,marginBottom:14}}>
          {([["tl","TL"],["usd","USD"],["eur","EUR"],["altin","Altın"]] as const).map(([v,l])=>(
            <button key={v} onClick={()=>setSiralamaParaBirimi(v)} style={{
              flex:1,padding:"9px 4px",borderRadius:10,border:`1.5px solid ${siralamaParaBirimi===v?C.blue:C.border}`,
              background:siralamaParaBirimi===v?C.blueLight:C.card,color:siralamaParaBirimi===v?C.blue:C.sub,
              fontWeight:siralamaParaBirimi===v?700:500,fontSize:12,cursor:"pointer"
            }}>{l}</button>
          ))}
        </div>
      )}

      {!yukleniyor&&!hata&&bankalarSirali.map((b:any,i:number)=>(
        <Card key={i}>
          <p style={{margin:"0 0 10px",fontSize:14,fontWeight:800,color:C.label}}>{b.ad}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {([["TL",b.tl,maxTl],["USD",b.usd,maxUsd],["EUR",b.eur,maxEur],["Altın",b.altin,maxAltin]] as const).map(([etiket,deger,maxDeger])=>(
              <div key={etiket} style={{background:deger!=null&&deger===maxDeger?C.greenLight:"rgba(255,255,255,0.04)",border:deger!=null&&deger===maxDeger?`1px solid ${C.green}`:"1px solid transparent",borderRadius:10,padding:"8px 10px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <span style={{fontSize:10.5,color:C.sub,fontWeight:700}}>{etiket}</span>
                  {deger!=null&&deger===maxDeger&&<span style={{fontSize:9,fontWeight:800,color:C.green,background:"rgba(74,222,128,0.18)",padding:"1px 6px",borderRadius:20}}>EN YÜKSEK</span>}
                </div>
                <p style={{margin:"2px 0 0",fontSize:16,fontWeight:800,color:deger!=null&&deger===maxDeger?C.green:C.label,fontFamily:"monospace"}}>
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      ...
      <Card>
        ...
        {doviz!=="TL"&&(
          <div onClick={()=>setYpBsmvMuaf(v=>!v)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 12px",marginTop:2,borderRadius:10,background:ypBsmvMuaf?C.greenLight:"rgba(255,255,255,0.04)",border:`1px solid ${ypBsmvMuaf?C.green:C.border}`,cursor:"pointer"}}>
            <div>
        ...
        {screen==="home"?(
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:20}}>
              ...
              <button onClick={()=>setBildirimAcik(true)} style={{
                display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,flexShrink:0,
                border:"none",background:"transparent",cursor:"pointer",padding:0,
              }}>
                <Bell size={20} color="#9FC1EA" strokeWidth={2} absoluteStrokeWidth/>
                <span style={{fontSize:8,fontWeight:700,color:"#9FC1EA",lineHeight:1.1,whiteSpace:"nowrap"}}>Hata ve Öneri Bildir</span>
              </button>
            </div>
