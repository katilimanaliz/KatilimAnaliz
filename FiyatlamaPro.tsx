...
const API_BASE = IS_NATIVE ? "https://katilimplus.com" : "";
...
function App(){
  ...
  const pushDebugEkle=(msg:string)=>setPushDebugLog(prev=>[...prev, msg]);
  ...
  //
  // GEÇİCİ DEBUG MODU: alert() Android WebView'de görünmeyebildiği için
  // bunun yerine ekranda görünen bir log listesi kullanıyoruz.
  // Sorun çözüldükten sonra bu debug bloğu kaldırılmalı.
  useEffect(()=>{
    ...
    pushDebugEkle(`Başlangıç kontrolü — modül-seviye IS_NATIVE: ${IS_NATIVE} | çalışma-anı kontrolü: ${gercekIsNative} | window.Capacitor var mı: ${!!(window as any).Capacitor}`);

    if(!gercekIsNative){ pushDebugEkle("gercekIsNative = false, native değil, atlanıyor."); return; }
    pushDebugEkle("useEffect başladı, native onaylandı.");

    PushNotifications.addListener("registration", (token) => {
      console.log("Push registration token:", token.value);
      pushDebugEkle("BAŞARILI: Token alındı (" + token.value.slice(0,12) + "...), gönderiliyor...");
      fetch(`${API_BASE}/api/bildirim?islem=kaydet`, {
        ...
        pushDebugEkle(`Backend cevabı: HTTP ${r.status}`);
      }).catch((e) => {
        pushDebugEkle("Token gönderilirken HATA: " + String(e));
    ...
    PushNotifications.addListener("registrationError", (err) => {
      ...
      pushDebugEkle("KAYIT HATASI (registrationError): " + JSON.stringify(err));
    });
    ...
    (async () => {
      try {
        ...
        pushDebugEkle("İzin durumu: " + JSON.stringify(izin));
        if (izin.receive === "prompt") {
          izin = await PushNotifications.requestPermissions();
          pushDebugEkle("İstek sonrası izin durumu: " + JSON.stringify(izin));
        }
        if (izin.receive !== "granted") {
          pushDebugEkle("İzin verilmedi, register() çağrılmıyor. Durum: " + izin.receive);
          return;
        }
        pushDebugEkle("register() çağrılıyor...");
        await PushNotifications.register();
        pushDebugEkle("register() çağrısı tamamlandı, token event'i bekleniyor...");
      } catch (e) {
        pushDebugEkle("GENEL HATA (try/catch): " + String(e));
    ...
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif",background:"#0F1923",minHeight:"100dvh",maxWidth:IS_NATIVE?680:430,margin:"0 auto"}}>
      {pushDebugLog.length>0 && (
        <div style={{
          ...
          background:"rgba(0,0,0,0.92)", color:"#4CFF7A",
          fontFamily:"monospace", fontSize:11, lineHeight:1.5,
          padding:"10px 12px", borderBottom:"2px solid #4CFF7A",
        }}>
          <div style={{color:"#fff",fontWeight:700,marginBottom:6}}>🔧 PUSH DEBUG LOG (geçici):</div>
          {pushDebugLog.map((satir,i)=>(
            <div key={i} style={{marginBottom:4,wordBreak:"break-all"}}>{i+1}. {satir}</div>
          ))}
          <div onClick={()=>setPushDebugLog([])} style={{marginTop:8,color:"#fff",background:"#333",display:"inline-block",padding:"4px 10px",borderRadius:6,fontSize:11}}>Kapat</div>
        </div>
      )}
          ...
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:20}}>
              ...
              <button onClick={()=>setBildirimAcik(true)} style={{
                ...
                <span style={{fontSize:8,fontWeight:700,color:"#9FC1EA",lineHeight:1.1,whiteSpace:"nowrap"}}>Hata Öneri Bildir</span>
              </button>
            </div>
