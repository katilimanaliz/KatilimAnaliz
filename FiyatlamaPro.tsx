...
function KarPayiOranlari({nav}:{nav:any}){
  ...
  useEffect(()=>{
    fetch("/api/kar-payi")
      ...
      .then(d=>{ if(d.success) setVeri(d); else setHata(d.error||"Veri alınamadı"); })
    ...
    <div style={{padding:"0 16px 32px"}}>
      <Card>
        <p style={{margin:0,fontSize:12,color:C.sub,lineHeight:1.6}}>
          100 Bin TL / 100 Bin USD / 100 Bin EUR / 1.000gr Altın, 1 aylık vade, şubeden açılan standart hesaba dağıtılan brüt yıllık kâr payı oranı (%). Kaynak: bankaların kendi ilanları — TKBB metodolojisiyle aynı. Kâr payı oranları ileriye yönelik bir taahhüt değildir, gösterge niteliğindedir.
        </p>
      ...
      {!yukleniyor&&!hata&&bankalar.map((b:any,i:number)=>(
        <Card key={i}>
      ...
      <button onClick={()=>nav("karPayiGuncelle")} style={{width:"100%",marginTop:8,padding:"12px",borderRadius:12,border:`1px solid ${C.border}`,background:"transparent",color:C.sub,fontSize:12,fontWeight:600,cursor:"pointer"}}>
        🔒 Kâr Payı Güncelle (Yetkili)
      </button>
    </div>
  );
}

// ── Kâr Payı Güncelle (şifreli, sadece yetkili) ─────────────────────────────
function KarPayiGuncelle({nav}:{nav:any}){
  ...
  useEffect(()=>{
    fetch("/api/kar-payi")
      .then(r=>r.json())
      .then(d=>{ if(d.success) setBankalar(d.bankalar||[]); })
      .catch(()=>{})
      .finally(()=>setYukleniyor(false));
  },[]);

  const alanGuncelle=(i:number,alan:string,deger:string)=>{
    setBankalar(prev=>prev.map((b,idx)=>idx===i?{...b,[alan]:deger}:b));
  };
  ...
  const kaydet=async()=>{
    ...
    setMesaj(null);
    try{
      const r=await fetch("/api/kar-payi",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({sifre:sifreGirisi,bankalar}),
      });
      const d=await r.json().catch(()=>null);
      if(!r.ok||!d?.success){
        setMesaj({tip:"hata",metin:d?.error||`HTTP ${r.status}`});
      } else {
        setBankalar(d.bankalar||bankalar);
        setMesaj({tip:"basari",metin:"Kaydedildi ✓"});
      }
    }catch(e:any){
      setMesaj({tip:"hata",metin:e?.message||"Bağlantı hatası"});
    }finally{
  ...
  if(!dogrulandi){
    return(
      <div style={{padding:"0 16px 32px"}}>
        <Card>
          <SecTitle>Yetkili Girişi</SecTitle>
          <Field label="Şifre" value={sifreGirisi} onChange={setSifreGirisi} type="password" suffix=""/>
          <button
            onClick={()=>setDogrulandi(true)}
            disabled={!sifreGirisi}
            style={{width:"100%",marginTop:10,padding:"12px",borderRadius:12,border:"none",background:sifreGirisi?C.blue:"rgba(255,255,255,0.08)",color:sifreGirisi?"#fff":C.sub,fontWeight:700,fontSize:14,cursor:sifreGirisi?"pointer":"default"}}
          >
            Devam Et
          </button>
          <p style={{margin:"8px 0 0",fontSize:10.5,color:C.sub}}>Şifre burada doğrulanmaz — kaydet'e bastığınızda sunucu tarafında kontrol edilir. Yanlışsa hata mesajı gösterilir.</p>
        </Card>
      </div>
    );
  }
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      <Card>
        <p style={{margin:0,fontSize:12,color:C.sub,lineHeight:1.6}}>
          Her banka için 100 Bin TL / 100 Bin USD / 100 Bin EUR / 1.000gr Altın, 1 aylık vade brüt yıllık kâr payı oranını (%) girin. Değişmeyen bankalarda önceki değer zaten dolu geliyor, sadece değişenleri güncelleyin. Boş bırakılan alan "—" olarak gösterilir.
        </p>
      </Card>

      {yukleniyor&&<Card><p style={{margin:0,fontSize:13,color:C.sub,textAlign:"center",padding:"20px 0"}}>Yükleniyor…</p></Card>}

      {!yukleniyor&&bankalar.map((b,i)=>(
        <Card key={i}>
          <p style={{margin:"0 0 10px",fontSize:14,fontWeight:800,color:C.label}}>{b.ad}</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <Field label="TL" value={b.tl??""} onChange={(v:string)=>alanGuncelle(i,"tl",v)} suffix="%"/>
            <Field label="USD" value={b.usd??""} onChange={(v:string)=>alanGuncelle(i,"usd",v)} suffix="%"/>
            <Field label="EUR" value={b.eur??""} onChange={(v:string)=>alanGuncelle(i,"eur",v)} suffix="%"/>
            <Field label="Altın" value={b.altin??""} onChange={(v:string)=>alanGuncelle(i,"altin",v)} suffix="%"/>
          </div>
        </Card>
      ))}

      {mesaj&&(
        <div style={{background:mesaj.tip==="basari"?C.greenLight:"rgba(248,113,113,0.12)",border:`1px solid ${mesaj.tip==="basari"?C.green:C.red}`,borderRadius:10,padding:"10px 12px",marginBottom:10}}>
          <p style={{margin:0,fontSize:12.5,fontWeight:700,color:mesaj.tip==="basari"?C.green:C.red}}>{mesaj.tip==="basari"?"✓ ":"⚠️ "}{mesaj.metin}</p>
        </div>
      )}
      ...
      <button
        ...
        style={{width:"100%",padding:"14px",borderRadius:12,border:"none",background:kaydediliyor?"rgba(255,255,255,0.08)":C.blue,color:kaydediliyor?C.sub:"#fff",fontWeight:800,fontSize:15,cursor:kaydediliyor?"default":"pointer"}}
      >
...
async function asistanPiyasaOzeti():Promise<string>{
  ...
  try{
    const [haberD,kurD]=await Promise.all([
      fetch("/api/finans-haberleri").then(r=>r.ok?r.json():null).catch(()=>null),
      fetch("/api/piyasa-fiyatlar?tip=kur").then(r=>r.ok?r.json():null).catch(()=>null),
    ]);
...
function Asistan({nav, settings}:{nav:any, settings?:any}){
  ...
  const send=async()=>{
    ...
    try{
      ...
      const r=await fetch("/api/asistan-ai",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
...
function FinansalGostergeler({onKurTikla}:any){
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      fetchTimeout('/api/piyasa-fiyatlar?tip=altin', 4000),
    ]).then(([d, altin])=>{
      if(d){
        setKurlar([
    ...
    Promise.all([
      fetchTimeout('/api/piyasa-fiyatlar?tip=kripto', 4000),
      fetchTimeout('/api/piyasa-fiyatlar?tip=petrol', 4000),
...
const MENU = {
  ...
  karPayiGuncelle:{title:"Kâr Payı Güncelle",back:"karPayiOranlari"},
...
function HtDovizDonusturucu(){
  ...
  useEffect(()=>{
    fetch("/api/piyasa-fiyatlar?tip=kur").then(r=>r.ok?r.json():null).then(d=>{
      if(d){
        setKurlar({
...
const fetchKurlarViaClaudeAPI = async () => {
  try {
    const res = await fetch("/api/piyasa-fiyatlar?tip=kur");
  ...
  return(
    ...
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif",background:"#0F1923",minHeight:"100dvh",maxWidth:430,margin:"0 auto"}}>
      ...
      <KategoriRenkContext.Provider value={EKRAN_KATEGORI[screen]||null}>
        ...
        {screen==="karPayiGuncelle"&&<KarPayiGuncelle nav={nav}/>}
