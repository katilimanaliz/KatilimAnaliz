...
function GetiriKarsilastirma(){
    ...
    fetch(`${API_BASE}/api/tefas-proxy`)
      ...
      .then(j=>{
        ...
        const ort=(alan:string)=>{
          const v=fonlar.map((f:any)=>f?.[alan]).filter((x:any)=>typeof x==="number"&&isFinite(x)&&x!==0);
  ...
  const barlar=useMemo(()=>{
    ...
    if(fonAlan && fonOrt[fonAlan]!=null){
      liste.push({ad:"Katılım Fonları (Ort.)",getiri:Math.round(fonOrt[fonAlan]*100)/100});
    }
      ...
      {yukleniyor&&barlar.length===0?(
        ...
        <>
          ...
          <p style={{margin:"12px 4px 0",fontSize:10,color:"rgba(255,255,255,0.3)",lineHeight:1.5}}>
            Kaynak: Yahoo Finance (kur, maden, endeks) ve TEFAS/Fonoloji (fon ortalaması). Gram fiyat getirileri ons ve USD/TRY bileşiminden hesaplanır. Fon ortalaması 6 Ay döneminde veri kaynağında bulunmadığından gösterilmez. Yatırım tavsiyesi değildir.
          </p>
        </>
      )}
    </div>
...
const TAB_OF_SCREEN:any = {
  ...
  araclarMenu:"araclar", sozluk:"araclar", vadeTakibi:"araclar", katilimBankalari:"araclar", getiriKarsilastirma:"araclar",
  asistan:"yapayzeka",
  profil:"profil",
};
...
function App(){
  ...
  const kolonW=IS_NATIVE?680:(genisEkran?Math.max(940,Math.min(1240,Math.round(ekranW*0.74))):430);
  const icerikOlcek=!IS_NATIVE&&ekranW>=1600?1.22:(genisEkran&&ekranW>=1280?1.12:1);
