...
function VerimlilikAnalizi({s, evdsMakro}:any){
  ...
  const r=useCallback(()=>{
    ...
    const nemaOranPct = (doviz==="TL" && aofm!=null) ? aofm*0.86 : null;
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      ...
      {r&&r.brutFaiz!==undefined&&<Card>
        ...
        {doviz==="YP" ? (
          <p style={{margin:"4px 0 0",fontSize:10.5,color:C.sub}}>YP zorunlu karşılıklara TCMB nema ödemez, bu yüzden YP'de nema getirisi hesaplanmaz.</p>
        ) : r.nemaGetirisi!=null ? (
...
function HaftalikPiyasaOzeti(){
  ...
  const satirlar=gosterilen?.satirlar||[];
  ...
  // Otomatik yorum — tamamen tablo verisinden üretilir
  const yorum=useMemo(()=>{
    ...
    const p=[];
    p.push(`${donemBas&&donemSon?`${donemBas} – ${donemSon} haftasında`:"Bu hafta"} takip edilen ${satirlar.length} enstrümanın ${yukselen} tanesi değer kazandı.`);
    p.push(`Haftanın en güçlü performansı ${(enIyi.getiri>=0?"+":"")}${enIyi.getiri.toFixed(2).replace(".",",")}% ile ${enIyi.ad} tarafında gerçekleşti.`);
    if(enKotu.getiri<0){
      p.push(`En zayıf görünüm ise ${enKotu.getiri.toFixed(2).replace(".",",")}% ile ${enKotu.ad} oldu.`);
    } else {
      p.push(`En düşük getiri dahi pozitif bölgede kaldı (${enKotu.ad}: +${enKotu.getiri.toFixed(2).replace(".",",")}%).`);
    }
    const usd=satirlar.find((g:any)=>g.kod==="USDTRY");
    const xu=satirlar.find((g:any)=>g.kod==="XU100");
    if(usd&&xu){
      if(xu.getiri>usd.getiri) p.push(`BIST 100 (${(xu.getiri>=0?"+":"")}${xu.getiri.toFixed(2).replace(".",",")}%), dolar kurunun (${(usd.getiri>=0?"+":"")}${usd.getiri.toFixed(2).replace(".",",")}%) üzerinde getiri sağladı.`);
      else p.push(`Dolar kuru (${(usd.getiri>=0?"+":"")}${usd.getiri.toFixed(2).replace(".",",")}%), hafta genelinde BIST 100'ün (${(xu.getiri>=0?"+":"")}${xu.getiri.toFixed(2).replace(".",",")}%) önünde yer aldı.`);
    }
    if(fonHafta!=null){
      p.push(`Katılım esaslı para piyasası fonlarının haftalık ortalama getirisi ${(fonHafta>=0?"+":"")}${fonHafta.toFixed(2).replace(".",",")}% düzeyinde gerçekleşti.`);
    }
    return p.join(" ");
      ...
      {yukleniyor&&satirlar.length===0?(
        ...
        <>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,overflow:"hidden",marginBottom:14}}>
            ...
            {satirlar.map((g:any,i:number)=>{
              ...
              return(
                <div key={g.kod} style={{display:"flex",alignItems:"center",padding:"11px 14px",borderBottom:i<satirlar.length-1?"1px solid rgba(255,255,255,0.05)":"none"}}>
                  ...
                  <span style={{width:82,textAlign:"right",fontSize:12,fontFamily:"monospace",fontWeight:700,color:"#fff"}}>{fmtFiyat(g.son)}</span>
            ...
            {fonHafta!=null&&(
              <div style={{display:"flex",alignItems:"center",padding:"11px 14px",borderTop:"1px solid rgba(255,255,255,0.05)"}}>
                <span style={{flex:1,minWidth:0,fontSize:12.5,fontWeight:700,color:"#E8F0FA",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>Para Piyasası Fonları (Ort.)</span>
                <span style={{width:82,textAlign:"right",fontSize:12,fontFamily:"monospace",color:"rgba(255,255,255,0.35)"}}>—</span>
                <span style={{width:82,textAlign:"right",fontSize:12,fontFamily:"monospace",color:"rgba(255,255,255,0.35)"}}>—</span>
                <span style={{width:74,textAlign:"right",fontSize:12,fontWeight:800,color:fonHafta>=0?"#4ADE80":"#F87171"}}>
                  {fonHafta>=0?"▲":"▼"} %{Math.abs(fonHafta).toFixed(2).replace(".",",")}
                </span>
              </div>
            )}
          </div>
          ...
          {yorum&&(
            <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,padding:"14px 16px"}}>
              ...
              <p style={{margin:0,fontSize:13,color:"rgba(255,255,255,0.75)",lineHeight:1.65}}>{yorum}</p>
            </div>
          )}

          <p style={{margin:"12px 4px 0",fontSize:10,color:"rgba(255,255,255,0.3)",lineHeight:1.5}}>
            Kaynak: Yahoo Finance (bir önceki tamamlanmış haftanın Pazartesi–Cuma kapanışları; yeni hafta verisi Cumartesi günü yayınlanır) ve TEFAS/Fonoloji. Gram fiyatları ons ve USD/TRY üzerinden hesaplanır. Özet metni tablodaki verilerden otomatik üretilir; yatırım tavsiyesi değildir.
          </p>
        </>
      )}
    </div>
...
const MENU = {
  ...
  getiriKarsilastirma:{title:"Getiri Karşılaştırma",back:"araclarMenu"},
  haftalikOzet:{title:"Haftalık Piyasa Özeti",back:"araclarMenu"},
...
function htParaKuru(baz:string, karsi:string, kurD:any):number|null{
  const tryKarsiligi=(p:string)=>p==="USD"?kurD?.USD_TRY:p==="EUR"?kurD?.EUR_TRY:null; // 1 X = ? TRY
...
function HtForwardHesaplama(){
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      if(kur) setSpotKur(String(kur).replace(".",",")); else sorunlar.push("Spot kur alınamadı (/api/piyasa-fiyatlar?tip=kur)");
...
function App(){
  ...
  // Kolon genişliği artık ekrana ORANLI (otomatik): geniş ekranda ekranın
  // ~%74'ünü kaplar (alt sınır 940, üst sınır 1240). Ayrıca çok geniş
  // monitörlerde içerik hafifçe büyütülür (zoom) — böylece tek kolon tasarım
  // gerilmeden ekranı daha dolu kullanır.
  const kolonW=IS_NATIVE?680:(genisEkran?Math.max(940,Math.min(1400,Math.round(ekranW*0.86))):430);
  const icerikOlcek=!IS_NATIVE&&ekranW>=1600?1.30:(genisEkran&&ekranW>=1280?1.18:1);
  const altBarW=IS_NATIVE?560:(genisEkran?Math.max(620,Math.min(760,Math.round(kolonW*0.62))):402);
  useEffect(()=>{
    if(IS_NATIVE) return;
