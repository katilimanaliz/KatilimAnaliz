...
function FonGetiriIzleme({ settings }) {
  ...
  const [sirala,      setSirala]     = useState<string>("yillik");
    ...
    return siralanmis
      .filter(f=>{
        ...
        const yOk=filtreYon==="Tümü"||(
              filtreYon==="Hisse"?((f.kategori||"").includes("Hisse")||(f.ad||"").includes("HİSSE")):
              filtreYon==="Para"?((f.kategori||"").includes("Para")||(f.ad||"").includes("PARA")):
              filtreYon==="Borçlanma"?((f.kategori||"").includes("Borçlanma")||(f.ad||"").includes("BORÇ")):
              filtreYon==="Karma"?((f.kategori||"").includes("Değişken")||(f.ad||"").includes("DEĞİŞKEN")):
              filtreYon==="Sepet"?((f.kategori||"").includes("Sepet")||(f.ad||"").includes("SEPET")):
              filtreYon==="Altın"?((f.kategori||"").includes("Altın")||(f.ad||"").includes("ALTIN")):
              filtreYon==="Endeks"?((f.kategori||"").includes("Endeks")||(f.ad||"").includes("ENDEKS")):
              f.yonetici.trim()===filtreYon
            );
        return aOk&&yOk;
      })
;
  ...
  return (
    <div style={{background:FC.bg,fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif",color:FC.text,minHeight:"100%"}}>
      ...
      <div style={{display:"flex",background:"rgba(255,255,255,0.04)",padding:"6px 12px",gap:4,overflowX:"auto",borderBottom:`1px solid ${FC.border}`}}>
        {PERIODS.map(p=>(
          <button key={p.key} onClick={()=>setAktifPeriod(p.key)} style={{
      ...
      {/* Tablo başlığı */}
      <div style={{display:"flex",alignItems:"center",padding:"6px 10px",background:"#111820",borderBottom:`1px solid ${FC.border}`,position:"sticky" as any,top:0,zIndex:10}}>
        <span style={{width:36,flexShrink:0,fontSize:9,fontWeight:700,color:FC.sub,letterSpacing:0.5}}>KOD</span>
        <span style={{flex:1,fontSize:9,fontWeight:700,color:FC.sub,letterSpacing:0.5}}>FON ADI <span style={{fontWeight:400,opacity:0.5}}>(dokunun)</span></span>
        <span style={{width:60,textAlign:"right",flexShrink:0,fontSize:9,fontWeight:700,color:FC.sub,letterSpacing:0.5}}>KATEGORİ</span>
        {(["gunluk","aylik","yillik","portfoy","yatirimci"] as const).map(k=>(
          <span key={k} onClick={()=>setSirala((ss:string)=>ss===k?k+"D":k)}
            style={{width:k==="portfoy"?58:k==="yatirimci"?52:44,textAlign:"right",flexShrink:0,
              fontSize:9,fontWeight:700,letterSpacing:0.5,cursor:"pointer",
              color:sirala?.replace("D","")=== k?FC.green:FC.sub}}>
            {k==="gunluk"?"1G":k==="aylik"?"1AY":k==="yillik"?"1Y ▼":k==="portfoy"?"BÜYÜKLÜK":"YATIRIMCI"}
          </span>
        ))}
      </div>
              ...
              return (
                <div key={fon.kod}>
                  ...
                  <div onClick={()=>handleFonTikla(fon)} style={{
                    ...
                    <span style={{width:36,flexShrink:0,fontSize:12,fontWeight:800,color:vakif?FC.green:FC.blue}}>
                    ...
                    <span style={{width:60,textAlign:"right",flexShrink:0,fontSize:8,color:FC.sub,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                      {(fon.kategori||"").length>12?(fon.kategori||"").slice(0,12)+"…":(fon.kategori||"—")}
                    </span>
                    <span style={{width:44,textAlign:"right",flexShrink:0,fontSize:11,fontWeight:700,color:pctCol(fon.gunluk)}}>
                      {fon.gunluk==null||fon.gunluk===0?"—":(fon.gunluk>0?"+":"")+fon.gunluk.toFixed(2)+"%"}
                    </span>
                    <span style={{width:44,textAlign:"right",flexShrink:0,fontSize:11,fontWeight:700,color:pctCol(fon.aylik)}}>
                      {fon.aylik==null?"—":(fon.aylik>0?"+":"")+fon.aylik.toFixed(2)+"%"}
                    </span>
                    <span style={{width:44,textAlign:"right",flexShrink:0,fontSize:11,fontWeight:700,color:pctCol(fon.yillik)}}>
                      {fon.yillik==null?"—":(fon.yillik>0?"+":"")+fon.yillik.toFixed(2)+"%"}
                    </span>
                    <span style={{width:58,textAlign:"right",flexShrink:0,fontSize:10,color:FC.sub}}>{fmtPF(fon.portfoy)}</span>
                    <span style={{width:52,textAlign:"right",flexShrink:0,fontSize:10,color:FC.sub}}>{fon.yatirimci?fon.yatirimci.toLocaleString("tr-TR"):"—"}</span>
                  </div>
                  ...
                  {sel&&(
                    <div style={s.detayWrap}>
                      ...
                      <div style={{background:"#1a2535",borderRadius:10,padding:"10px 12px",marginBottom:10,border:`1px solid ${FC.border}`}}>
                        <div style={{fontSize:11,color:FC.sub,marginBottom:3}}>Fon Adı</div>
...
function HisseDetay({ hisse, onGeri }: { hisse: any, onGeri: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`https://www.isyatirim.com.tr/_layouts/15/Isyatirim.Website/Common/Data.aspx/HisseTekil?hisse=${hisse.ticker}&startdate=${bas}&enddate=${bitis}`)
      .then(r => r.json())
      .then(d => {
        const pts = (d.value || []).map((p: any) => ({
          tarih: p.HGDG_TARIH?.slice(0,10) || "",
          fiyat: parseFloat(p.HGDG_KAPANIS) || 0,
        })).filter((p: any) => p.fiyat > 0);
        setGrafik(pts);
      })
      .catch(() => setGrafik([]))
  ...
  const SVGGrafik = () => {
    if (grafik.length < 2) return <div style={{height:120,display:"flex",alignItems:"center",justifyContent:"center",color:C.sub,fontSize:12}}>Veri yüklenemedi</div>;
    const min = Math.min(...grafik.map(p => p.fiyat));
    const max = Math.max(...grafik.map(p => p.fiyat));
    const w = 320, h = 100;
    const pts = grafik.map((p, i) => {
      const x = (i / (grafik.length - 1)) * w;
      const y = h - ((p.fiyat - min) / (max - min || 1)) * h;
      return `${x},${y}`;
    }).join(" ");
    const ilk = grafik[0]?.fiyat;
    const son = grafik[grafik.length - 1]?.fiyat;
    const renk2 = son >= ilk ? C.green : C.red;
    return (
      <svg viewBox={`0 0 ${w} ${h}`} style={{width:"100%",height:120,overflow:"visible"}}>
        <polyline points={pts} fill="none" stroke={renk2} strokeWidth="2" strokeLinejoin="round"/>
        <circle cx={(grafik.length-1)/(grafik.length-1)*w} cy={h - ((son-min)/(max-min||1))*h} r="4" fill={renk2}/>
      </svg>
    );
  };
    ...
    <div style={{background:C.bg, padding:"12px 14px 80px", minHeight:"100%"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontSize:15,fontWeight:700,color:C.text}}>BIST Verileri</div>
...
const MENU = {
  ...
  bistHisseTarayici:{title:"BIST Verileri",back:"home"},
