...
function GetiriKarsilastirma(){
  ...
  // Piyasa getirileri — aralık değişince çek (10 dk sessionStorage cache)
  useEffect(()=>{
    let iptal=false;
    const CK="gk_"+aralik;
    try{
    ...
    setHata(null);
    fetch(`${API_BASE}/api/getiri?aralik=${aralik}`)
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(iptal) return;
        if(d?.basarili){
  ...
  // Katılım fonları ortalaması — bir kere çek (tüm dönem alanları tek yanıtta)
  useEffect(()=>{
    fetch(`${API_BASE}/api/tefas-proxy`)
      .then(r=>r.ok?r.json():null)
      .then(j=>{
  ...
  const barlar=useMemo(()=>{
    const liste:{ad:string,getiri:number}[]=[];
  ...
  return(
    <div style={{background:"#0F1923",minHeight:"100vh",padding:"0 16px 40px",boxSizing:"border-box"}}>
      {/* Dönem seçici */}
      <div style={{display:"flex",gap:8,marginBottom:16,overflowX:"auto",paddingBottom:2}} className="piyasa-scroll">
        {GK_ARALIKLAR.map(a=>(
...
function App(){
  ...
  return(
    <>
      ...
      `}</style>
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif",background:"#0F1923",minHeight:"100dvh",maxWidth:IS_NATIVE?680:430,margin:"0 auto"}}>
      ...
      {["home","hesaplaMenu","piyasaMenu","araclarMenu","asistan","profil"].includes(screen)&&(
        <div style={{
          ...
          <div style={{
            width:"100%",maxWidth:IS_NATIVE?560:402,
