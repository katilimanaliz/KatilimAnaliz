...
function OdemePlani({plan, bsmvOran, kkdfOran, onClose, showKomisyon, basitOran, efektifOran, anaparaTutar, taksitAraligiGun}){
  ...
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
      <div style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",height:"96dvh",maxHeight:"96dvh",display:"flex",flexDirection:"column"}}>
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      ...
      {mod==="standart"&&r&&!r.vadeAsim&&r.pmt&&<Card>
        ...
        <RaporButon baslik="Yatırım Fonu Finansmanı" plan={r.plan} satirlar={raporSatirlari} bsmvOran={s.bireyselBSMV} kkdfOran={s.bireyselKKDF}/>
          </>);
        })()}
      </Card>}
...
function GostergeTabloModal({ad,seri,birim,onClose}:{ad:string,seri:{tarih:string,deger:number}[],birim?:string,onClose:()=>void}){
  ...
  return(
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",zIndex:600,display:"flex",alignItems:"flex-end"}} onClick={onClose}>
      <div style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"80vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
        <div style={{padding:"16px 20px 12px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div>
            <p style={{margin:0,fontSize:16,fontWeight:800,color:C.label}}>{ad}</p>
    ...
    <div style={{padding:"0 16px 32px"}}>
      <Card>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {([["USDTRY","USD/TRY"],["EURTRY","EUR/TRY"],["EURUSD","EUR/USD"]] as const).map(([v,l])=>(
...
function App(){
  ...
  const [piyasaGostergeGrafik,setPiyasaGostergeGrafik]=useState<any>(null);
  const [evdsMakro,setEvdsMakro]=useState<any>(null);
  ...
  return(
    ...
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Text',sans-serif",background:"#0F1923",minHeight:"100dvh",maxWidth:430,margin:"0 auto"}}>
      ...
      {/* Test aşaması uyarı şeridi — tam genişlik, ince, tıklanabilir. Sadece ana
          ekranda gösteriliyor (eskisiyle aynı kapsam), artık header içine sıkışmış
          bir köşe metni değil, kendi başına bir şerit. Opak koyu kehribar zemin
          bilerek seçildi (yarı şeffaf değil) — hangi arka planın önünde olursa
          olsun kontrast/okunabilirlik garantili olsun diye. */}
      {screen==="home"&&(
        <div onClick={()=>setBildirimAcik(true)} style={{
          background:"#2B2410",borderBottom:"1px solid rgba(255,214,10,0.3)",
          padding:"calc(8px + env(safe-area-inset-top,0px)) 20px 8px",
          display:"flex",alignItems:"center",gap:6,cursor:"pointer",
        }}>
          <span style={{fontSize:12,flexShrink:0}}>⚠️</span>
          <span style={{fontSize:10.5,fontWeight:800,color:"#FFD60A",flexShrink:0}}>Test aşamasındadır</span>
          <span style={{fontSize:10,color:"rgba(255,255,255,0.65)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>— hata ve önerilerini buradan bildirebilirsiniz</span>
        </div>
      )}
      {/* header */}
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
                ...
                <div style={{display:"flex",flexDirection:"column",justifyContent:"center",minWidth:0}}>
                  <span style={{fontSize:17,fontWeight:800,letterSpacing:"-0.01em",color:"#EAF1FA"}}>Katılım <span style={{color:"#1C9C7A"}}>Plus</span></span>
              ...
              {!kurulumBannerKapali&&(
                <div style={{position:"relative",flexShrink:0}}>
                  <button onClick={()=>setKurulumKilavuzuAcik(true)} style={{
                    display:"flex",flexDirection:"column",alignItems:"center",gap:3,
                    padding:"9px 12px",borderRadius:13,border:"1px solid rgba(91,155,216,0.3)",
                    background:"rgba(91,155,216,0.15)",cursor:"pointer",
                  }}>
                    <span style={{fontSize:18,lineHeight:1}}>📲</span>
                    <span style={{fontSize:8.5,fontWeight:800,color:"#9FC1EA",lineHeight:1.2,textAlign:"center",whiteSpace:"nowrap"}}>Ana Ekrana<br/>Ekleme Kılavuzu</span>
                  </button>
                  <button onClick={(e)=>{
                    e.stopPropagation();
                    setKurulumBannerKapali(true);
                    try{ localStorage.setItem("katilimAnaliz_kurulumBannerKapali_v1","1"); }catch{}
                  }} style={{position:"absolute",top:-6,right:-6,background:"#1C2A38",border:"1px solid rgba(255,255,255,0.15)",width:18,height:18,borderRadius:9,color:"rgba(255,255,255,0.6)",fontSize:10.5,lineHeight:1,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",padding:0}}>×</button>
                </div>
              )}
            </div>
                ...
                <div>
                  <div style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,overflow:"hidden",marginTop:8}}>
                    {GOSTERGELER.map((g,i)=>{
                      const tiklanabilir = g.seri && g.seri.length>0;
                      return(
                      <div key={i} onClick={()=>tiklanabilir&&setPiyasaGostergeGrafik({ad:g.seriAd,seri:g.seri,birim:g.seriBirim})} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 14px",borderBottom:i<GOSTERGELER.length-1?"1px solid rgba(255,255,255,0.06)":"none",cursor:tiklanabilir?"pointer":"default"}}>
                        <div>
                  ...
                  {piyasaGostergeGrafik&&<GostergeGrafikModal ad={piyasaGostergeGrafik.ad} seri={piyasaGostergeGrafik.seri} birim={piyasaGostergeGrafik.birim} onClose={()=>setPiyasaGostergeGrafik(null)}/>}
                </div>
              );
            })():(
              <>
