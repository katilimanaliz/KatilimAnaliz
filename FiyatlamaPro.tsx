...
💼 Konuyla ilgili kesin bilgi için bölge veya segment temsilcinizle iletişime geçebilirsiniz.`,tip:"bulunamadi",bulunanKisaltmalar};
  ...
  const send=async()=>{
    ...
    const modul=findModul(genisQ);
    const hesapIstiyor=asistanHesapNiyeti(q);

    const ekler=[];
    if(bulunanKisaltmalar&&bulunanKisaltmalar.length>0&&tip!=="bulunamadi"){
      ekler.push({tip:"oneri",text:`🔤 Anlaşılan kısaltma${bulunanKisaltmalar.length>1?"lar":""}: ${bulunanKisaltmalar.join(", ")}`});
    }
