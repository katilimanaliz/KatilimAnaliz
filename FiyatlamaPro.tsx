...
function BistHisseTarayici({ initialTicker, onInitialTuketildi }: { initialTicker?: string | null; onInitialTuketildi?: () => void } = {}) {
  ...
  const [siraGorunum, setSiraGorunum]   = useState<"yukselen"|"dusen"|"hacim">("yukselen");
    ...
    <div style={{background:C.bg, padding:"12px 14px 80px", minHeight:"100%"}}>
      ...
      <div style={{
        ...
        <div style={{display:"flex"}}>
          <div style={{flex:1}}>
            <div style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.5,marginBottom:3}}>Hisse</div>
            <div style={{fontSize:14.5,fontWeight:800,fontFamily:"monospace",color:"#fff"}}>{hisseIstatistik.toplam}</div>
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.5,marginBottom:3}}>Artan</div>
            <div style={{fontSize:14.5,fontWeight:800,fontFamily:"monospace",color:C.green}}>{hisseIstatistik.artan}</div>
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.5,marginBottom:3}}>Azalan</div>
            <div style={{fontSize:14.5,fontWeight:800,fontFamily:"monospace",color:C.red}}>{hisseIstatistik.azalan}</div>
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.5,marginBottom:3}}>Hacim</div>
      ...
      <div className="piyasa-scroll" style={{display:"flex",gap:8,overflowX:"auto",marginBottom:6,paddingBottom:2}}>
        {(["BIST 100","BIST 50","BIST 30"] as const).map(ad=>{
          ...
          <div key={ad}
            ...
            <div style={{fontSize:10,fontWeight:700,color:aktif?C.blue:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:0.4}}>{ad}</div>
      ...
      {/* Yükselenler / Düşenler / Hacim Segmenti */}
      <div style={{display:"flex",background:"rgba(255,255,255,0.05)",borderRadius:12,padding:3,marginBottom:10}}>
        {([["yukselen","Yükselenler","degisim1g",-1],["dusen","Düşenler","degisim1g",1],["hacim","Hacim","hacim",-1]] as const).map(([key,lbl,col,dir])=>(
          <div key={key} onClick={()=>{setSiraGorunum(key);setSiraBy(col as typeof siraBy);setSiraDir(dir);}} style={{
      ...
      <div style={{background:"#0F1923",padding:"12px 14px 92px",minHeight:"100%"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          <div style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:"11px 12px"}}>
            <p style={{margin:0,fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.4}}>Merkez</p>
            <p style={{margin:"3px 0 0",fontSize:13,fontWeight:700,color:"#fff"}}>{secili.merkez}</p>
          </div>
          <div style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:"11px 12px"}}>
            <p style={{margin:0,fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.4}}>Sermaye Yapısı</p>
            <p style={{margin:"3px 0 0",fontSize:11.5,fontWeight:700,color:"#fff",lineHeight:1.3}}>{secili.sermaye}</p>
          </div>
        </div>

        <p style={{margin:"0 0 8px",fontSize:11,fontWeight:800,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.5}}>Öne Çıkanlar</p>
...
const HESAPLA_KATEGORILER = [
  {id:"tumu",     label:"Tümü"},
  {id:"katilim",  label:"Katılım Hesabı"},
...
function PiyasaOzetiKart({ad,sembol,paraOnek,dec,onTikla}:{ad:string,sembol:string,paraOnek?:string,dec:number,onTikla:()=>void}){
  ...
  return(
    <div className="press-card" onClick={onTikla} style={{
      ...
      <p style={{margin:0,fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:0.2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ad}</p>
      {guncel!=null ? (
        <>
          <p style={{margin:"4px 0 2px",fontSize:15,fontWeight:800,color:"#fff",fontFamily:"monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
        ...
        {screen==="home"?(
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:16}}>
              ...
              <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:1}}>{selamlama} · {kisaTarihStr}</span>
            </div>
            <div style={{
              fontSize:32,fontWeight:700,fontStyle:"italic",letterSpacing:"-0.01em",marginBottom:6,
        ...
        {screen==="home"&&(
          <div style={{background:"#0F1923",padding:"10px 12px 0",paddingBottom:92,boxSizing:"border-box",display:"flex",flexDirection:"column",overflowY:"auto"}}>
            ...
            {yaklasanTakvim.length>0&&(
              <>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.5}}>Yaklaşan Takvim · 3 Gün</span>
        ...
        {screen==="hesaplaMenu"&&(()=>{
          ...
          <div style={{background:"#0F1923",padding:"12px 12px 0",paddingBottom:92,boxSizing:"border-box",overflowY:"auto"}}>
            ...
            {gecmis.length>0 && hesaplaAramaQ==="" && (hesaplaFiltre==="tumu") && (
              <div style={{marginBottom:14}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:0.5}}>Son Kullanılanlar</span>
