...
function Icon({k, size=22, color="#8FA8D8", strokeWidth=2}:{k:string, size?:number, color?:string, strokeWidth?:number}){
...
function BistHisseTarayici({ initialTicker, onInitialTuketildi }: { initialTicker?: string | null; onInitialTuketildi?: () => void } = {}) {
  ...
  return (
    <div style={{background:C.bg, padding:"12px 14px 80px", minHeight:"100%"}}>
      ...
      {/* Endeks Şeridi */}
      <div className="piyasa-scroll" style={{display:"flex",gap:8,overflowX:"auto",marginBottom:14,paddingBottom:2}}>
        {["BIST 100","BIST 30","BIST 50"].map(ad=>(
          <div key={ad} style={{flex:"0 0 108px",background:"rgba(255,255,255,0.05)",border:`1px solid ${C.border}`,borderRadius:14,padding:"10px 12px"}}>
            <div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:0.4}}>{ad}</div>
...
const MENU = {
  ...
  haftalikBulten:{title:"Haftalık Piyasa Bülteni",back:"home"},
...
const TAB_OF_SCREEN:any = {
  ...
  piyasaHaberleri:"piyasa", finansalGostergeler:"piyasa", haftalikBulten:"piyasa",
...
const MENU_ARAMA_LIST=[
  ...
  {key:"haftalikBulten",     label:"Haftalık Piyasa Bülteni",                     icon:"🗞️", grup:"Piyasa & Veriler"},
...
// Not: dışarıdan (aracı kurum vb.) telif korumalı yorum/bülten içeriği ÇEKMİYORUZ.
// Bülten, uygulamanın kendi /api/gecmis verisinden (BIST100, USD/TRY, EUR/TRY,
// Altın gram, Brent) haftalık % değişim hesaplanarak otomatik/orijinal üretilir.
const HAFTALIK_BULTEN_LISTESI = [
  {ad:"BIST 100",     sembol:"XU100.IS",   dec:0, birim:"₺", ikon:"📊"},
  {ad:"USD/TRY",      sembol:"USDTRY=X",   dec:2, birim:"₺", ikon:"💵"},
  {ad:"EUR/TRY",      sembol:"EURTRY=X",   dec:2, birim:"₺", ikon:"💶"},
  {ad:"Altın (Gram)", sembol:"GRAM_ALTIN", dec:2, birim:"₺", ikon:"🥇"},
  {ad:"Brent Petrol", sembol:"BZ=F",       dec:2, birim:"$", ikon:"🛢️"},
];

function HaftalikBulten(){
  const [veriler,setVeriler]=useState<Record<string,any>>({});
  const [yukleniyor,setYukleniyor]=useState(true);
  const [hata,setHata]=useState<string|null>(null);

  useEffect(()=>{
    let iptal=false;
    const CACHE_KEY="haftalik_bulten_v1";
    try{
      const raw=sessionStorage.getItem(CACHE_KEY);
      if(raw){
        const {data,ts}=JSON.parse(raw);
        if(data && Date.now()-ts<30*60*1000){ setVeriler(data); setYukleniyor(false); }
      }
    }catch{}

    // Tek seferde başarısız olursa (soğuk başlangıç / geçici ağ hatası) 2 kez daha dener —
    // PiyasaOzetiKart/PiyasaSatiri'deki aynı yeniden-deneme mantığı.
    const fetchIleTekrar = (sembol:string, kalanDeneme:number): Promise<any> =>
      fetch(`/api/gecmis?sembol=${encodeURIComponent(sembol)}`)
        .then(r=>r.ok?r.json():null)
        .then(d=>{
          if(d && d.guncelFiyat!=null) return d;
          if(kalanDeneme>0) return new Promise(res=>setTimeout(()=>res(fetchIleTekrar(sembol,kalanDeneme-1)),1800));
          return d;
        })
        .catch(()=>{
          if(kalanDeneme>0) return new Promise(res=>setTimeout(()=>res(fetchIleTekrar(sembol,kalanDeneme-1)),1800));
    ...
    Promise.all(HAFTALIK_BULTEN_LISTESI.map((item,i)=>
      new Promise(res=>setTimeout(res,i*250)).then(()=>fetchIleTekrar(item.sembol,2))
    ))
      .then(sonuclar=>{
        if(iptal) return;
        ...
        sonuclar.forEach((d,i)=>{
          map[HAFTALIK_BULTEN_LISTESI[i].sembol]=d;
          if(d && d.guncelFiyat!=null) herhangiBasarili=true;
        });
        setVeriler(map);
        setYukleniyor(false);
        // Hepsi başarısızsa cache'e YAZMA — aksi halde 30 dk boyunca boş ekran cache'lenir.
        if(herhangiBasarili){
          try{ sessionStorage.setItem(CACHE_KEY, JSON.stringify({data:map, ts:Date.now()})); }catch{}
        } else {
          setHata("Piyasa verileri şu an alınamadı, birazdan tekrar deneyin");
        }
      })
      .catch(()=>{ if(!iptal){ setHata("Piyasa verileri alınamadı"); setYukleniyor(false); } });

    return ()=>{iptal=true;};
  },[]);

  const haftaOncesiNoktasi=(noktalar:any[])=>{
    if(!noktalar||noktalar.length<2) return null;
    const sonTarih=new Date(noktalar[noktalar.length-1].tarih);
    const hedef=new Date(sonTarih); hedef.setDate(sonTarih.getDate()-7);
    let secili=noktalar[0];
    for(const n of noktalar){ if(new Date(n.tarih)<=hedef) secili=n; }
    return secili;
  };

  const satirlar = HAFTALIK_BULTEN_LISTESI.map(item=>{
    const d=veriler[item.sembol];
    const noktalar=d?.noktalar||[];
    const guncel=d?.guncelFiyat ?? (noktalar.length?noktalar[noktalar.length-1].fiyat:null);
    const eski=haftaOncesiNoktasi(noktalar);
    const degisim=(guncel!=null && eski && eski.fiyat) ? ((guncel-eski.fiyat)/eski.fiyat*100) : null;
    return {...item, guncel, degisim};
  });

  const hazir = satirlar.filter(s=>s.degisim!=null);

  const bugun=new Date();
  const haftaBaslangic=new Date(bugun); haftaBaslangic.setDate(bugun.getDate()-7);
  const MONTHS=['Oca','Şub','Mar','Nis','May','Haz','Tem','Ağu','Eyl','Eki','Kas','Ara'];
  const tarihStr=`${haftaBaslangic.getDate()} ${MONTHS[haftaBaslangic.getMonth()]} – ${bugun.getDate()} ${MONTHS[bugun.getMonth()]}`;

  const cumleUret=(s:any)=>{
    if(s.degisim==null) return null;
    if(Math.abs(s.degisim)<0.05) return `${s.ad} haftayı yatay tamamladı.`;
    const yon = s.degisim>=0 ? "yükseldi" : "geriledi";
    const on = Math.abs(s.degisim)>=3 ? (s.degisim>=0?"sert bir yükselişle ":"belirgin bir düşüşle "):"";
    return `${s.ad} ${on}haftalık bazda %${Math.abs(s.degisim).toFixed(2).replace(".",",")} ${yon}.`;
  };
  const paragraf = hazir.length ? hazir.map(cumleUret).filter(Boolean).join(" ") : null;

  const fmtDeger=(v:number,dec:number)=>new Intl.NumberFormat("tr-TR",{minimumFractionDigits:dec,maximumFractionDigits:dec}).format(v);
  ...
  return(
    <div style={{background:"#0B0E14",minHeight:"100dvh",paddingBottom:32}}>
      <div style={{background:"linear-gradient(135deg,#1A1A2E 0%,#16213E 100%)",padding:"16px 16px 14px",borderBottom:"2px solid #FF6B35"}}>
        <p style={{margin:"0 0 2px",fontSize:11,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:"0.07em"}}>Katılım Analiz</p>
        <h2 style={{margin:0,fontSize:18,fontWeight:800,color:"#fff",display:"flex",alignItems:"center",gap:8}}>
          🗞️ Haftalık Piyasa Bülteni
        </h2>
        <p style={{margin:"4px 0 0",fontSize:11,color:"rgba(255,255,255,0.55)"}}>{tarihStr}</p>
      </div>

      <div style={{padding:"14px 16px"}}>
        {yukleniyor && Object.keys(veriler).length===0 && (
          <div style={{textAlign:"center",padding:40,color:"#9CA3AF",fontSize:13}}>⏳ Haftalık veriler hesaplanıyor...</div>
        )}
        {hata && (
          <div style={{textAlign:"center",padding:20,color:"#F87171",fontSize:12}}>⚠️ {hata}</div>
        )}

        {!yukleniyor && paragraf && (
          <div style={{padding:"14px 16px",marginBottom:14,background:"#151823",borderRadius:12,borderLeft:"3px solid #FF6B35"}}>
            <p style={{margin:0,fontSize:11,fontWeight:700,color:"#FF6B35",textTransform:"uppercase",letterSpacing:0.4,marginBottom:6}}>Haftanın Özeti</p>
            <p style={{margin:0,fontSize:13.5,color:"#E5E7EB",lineHeight:1.6}}>{paragraf}</p>
          </div>
        )}

        <div style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,overflow:"hidden"}}>
          {satirlar.map((s,i)=>{
            const pozitif = s.degisim!=null && s.degisim>=0;
            const renk = s.degisim==null ? "rgba(255,255,255,0.3)" : pozitif ? "#4ADE80" : "#F87171";
            return(
              <div key={s.sembol} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",borderBottom:i<satirlar.length-1?"1px solid rgba(255,255,255,0.06)":"none"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <span style={{fontSize:16}}>{s.ikon}</span>
                  <div>
                    <p style={{margin:0,fontSize:13,fontWeight:700,color:"#E8F0FA"}}>{s.ad}</p>
                    <p style={{margin:"1px 0 0",fontSize:11,color:"rgba(255,255,255,0.4)",fontFamily:"monospace"}}>
                      {s.guncel!=null ? `${s.birim}${fmtDeger(s.guncel,s.dec)}` : "—"}
                    </p>
                  </div>
                </div>
                <span style={{fontSize:13,fontWeight:800,color:renk}}>
                  {s.degisim!=null ? `${pozitif?"+":""}${s.degisim.toFixed(2).replace(".",",")}%` : "—"}
                </span>
              </div>
            );
          })}
        ...
        <div style={{margin:"14px 0 0",padding:"10px 14px",background:"#151823",borderRadius:10}}>
          <p style={{margin:0,fontSize:10,color:"#6B7280",lineHeight:1.5}}>
            🗞️ Bu bülten, uygulama içindeki piyasa verilerinden (BIST, döviz, altın, petrol) haftalık değişim hesaplanarak otomatik oluşturulur. Yatırım tavsiyesi değildir.
          </p>
        </div>
      </div>
    </div>
      ...
      <div style={{background:"#0F1923",padding:"44px 20px 20px"}}>
        {screen==="home"?(
          <div>
            ...
            <div style={{fontSize:30,fontWeight:800,color:"#F1F5F9",letterSpacing:"-0.02em",marginBottom:6,fontFamily:"'Inter',ui-rounded,-apple-system,sans-serif"}}>Hoş geldin</div>
      ...
      <div style={{paddingTop:0}}>
          ...
          <div style={{background:"#0F1923",padding:"10px 12px 0",paddingBottom:92,boxSizing:"border-box",display:"flex",flexDirection:"column",overflowY:"auto"}}>
            ...
            <div className="piyasa-scroll" style={{display:"flex",overflowX:"auto",gap:8,marginBottom:14,scrollSnapType:"x mandatory",WebkitOverflowScrolling:"touch",paddingBottom:2}}>
              {favoriler.map(key=>{
                ...
                if(!item) return null;
                return(
                  <div className="press-card" key={key} onClick={()=>nav(item.key)} style={{
                    flex:"0 0 110px",scrollSnapAlign:"start",
                    background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",
                    borderRadius:16,padding:"16px 8px",cursor:"pointer",minHeight:122,
                    display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,
                  }}>
                    <div style={{height:56,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon k={item.key} size={30} color="#5B9BD8"/></div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              {[
                ...
                {key:"haftalikBulten",    icon:"🗞️", label:"Haftalık Piyasa Bülteni"},
                {key:"finansalTakvim",    icon:"📅", label:"Finansal Takvim"},
              ].map(c=>(
                <div className="press-card" key={c.key} onClick={()=>nav(c.key)} style={{
                  background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",
                  borderRadius:16,padding:"18px 10px",cursor:"pointer",minHeight:120,
                  display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:10,
                }}>
                <div style={{height:58,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon k={c.key} size={32} color="#5B9BD8"/></div>
        ...
        {screen==="piyasaMenu"&&(()=>{
          ...
          <div style={{background:"#0F1923",padding:"12px 12px 0",paddingBottom:92,boxSizing:"border-box",overflowY:"auto"}}>
            ...
            {piyasaTabloFiltre==="bulten"?(
              <div onClick={()=>nav("haftalikBulten")} style={{
                background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",
                borderRadius:14,padding:"16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer",marginTop:8,
              }}>
                <span style={{fontSize:22}}>🗞️</span>
                <div style={{flex:1}}>
                  <p style={{margin:0,fontSize:13,fontWeight:800,color:"#fff"}}>Haftalık Piyasa Bülteni</p>
                  <p style={{margin:"2px 0 0",fontSize:11,color:"rgba(255,255,255,0.45)"}}>BIST, döviz ve emtiada haftalık özet</p>
                </div>
        ...
        {screen==="haftalikBulten"&&<HaftalikBulten/>}
        ...
        {/* ── YASAL UYARI FOOTER ── */}
        {!["home","hesaplaMenu","piyasaMenu","araclarMenu","asistan","sozluk","finansalTakvim","finansalGostergeler","vadeTakibi","katilimBankalari","piyasaHaberleri","haftalikBulten","ayarlar","profil"].includes(screen)&&(
          <div style={{
