...
function Asistan({nav, settings}:{nav:any, settings?:any}){
  ...
  const MODUL_MAP=[
    ...
    {keys:["gösterge","tüfe","enflasyon","politika faizi"],screen:"finansalGostergeler",label:"Finansal Göstergeler"},
...
function FinansalGostergeler({onKurTikla}:any){
  ...
  const SABIT=[
    ...
    {kategori:"Referans Oranlar (BIST)",icon:"📐",color:"#8B5CF6",items:[
      ...
      {ad:"TLREFK (Katılım Bankacılığı, Tahmini)",
       deger:evdsMakro?.["TP.BISTTLREFK.KAPANIS"]?.deger!=null?`%${evdsMakro["TP.BISTTLREFK.KAPANIS"].deger.toFixed(2).replace(".",",")}`:"—",
       tarih:evdsMakro?.["TP.BISTTLREFK.KAPANIS"]?.tarih?`${evdsMakro["TP.BISTTLREFK.KAPANIS"].tarih} · TLREF-0,096 tahmini`:"BIST",
       canli:evdsMakro?.["TP.BISTTLREFK.KAPANIS"]!=null,
       seri:evdsMakro?.["TP.BISTTLREFK.KAPANIS_SERI"],seriAd:"TLREFK (Tahmini)"},
    ]},
...
function PiyasaOzetiKart({ad,sembol,paraOnek,dec,onTikla}:{ad:string,sembol:string,paraOnek?:string,dec:number,onTikla:()=>void}){
  ...
  return(
    <div className="press-card" onClick={onTikla} style={{
      ...
      borderRadius:14,padding:"11px 10px 9px",cursor:"pointer",minWidth:0,
      transition:"background-color 700ms ease, border-color 700ms ease, box-shadow 700ms ease",
      ...flashStil,
    }}>
      <p style={{margin:0,fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.45)",textTransform:"uppercase",letterSpacing:0.2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{TR(ad)}</p>
      {guncel!=null ? (
        <>
        ...
        {screen==="home"&&(
          <div style={{background:"#0F1923",padding:"10px 12px 0",paddingBottom:108,boxSizing:"border-box",display:"flex",flexDirection:"column",overflowY:"auto"}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
              ...
              <span onClick={()=>nav("finansalGostergeler")} style={{fontSize:11,fontWeight:700,color:"#3B82F6",cursor:"pointer"}}>Tümü ›</span>
            </div>
            ...
            {ALT_BAR_SEKMELERI.map(t=>{
              ...
              return (
                <div key={t.tab} onClick={()=>nav(t.key)} style={{
                  ...
                  padding:"12px 1px 11px",cursor:"pointer",overflow:"hidden",
                }}>
                  <div style={{
                    display:"flex",flexDirection:"column",alignItems:"center",gap:3,
                    padding:"5px 8px 6px",borderRadius:16,maxWidth:"100%",boxSizing:"border-box",
