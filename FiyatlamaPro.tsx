      ...
      <KategoriRenkContext.Provider value={EKRAN_KATEGORI[screen]||null}>
        ...
        {screen==="home"&&(
          <div style={{background:"#0F1923",padding:"10px 12px 0",paddingBottom:108,boxSizing:"border-box",display:"flex",flexDirection:"column",overflowY:"auto"}}>
            ...
            {/* Finansal Göstergeler — ana sayfa özeti */}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
            ...
            <div onClick={()=>{setPiyasaTabloFiltre("gostergeler");nav("piyasaMenu");}} style={{
              ...
              padding:"2px 16px",marginBottom:14,cursor:"pointer",
            }}>
              ...
              {[
                {ad:"TCMB Politika Faizi", deger:"%37,00", tarih:"Haziran 2026 · PPK"},
                {ad:"TÜFE (Yıllık)", deger:evdsMakro?.["TUFE_YILLIK"]?.deger!=null?`%${evdsMakro["TUFE_YILLIK"].deger.toFixed(2).replace(".",",")}`:"—", tarih:evdsMakro?.["TUFE_YILLIK"]?.tarih?`${evdsMakro["TUFE_YILLIK"].tarih} · canlı`:""},
                {ad:"TÜFE (Aylık)", deger:evdsMakro?.["TUFE_AYLIK"]?.deger!=null?`%${evdsMakro["TUFE_AYLIK"].deger.toFixed(2).replace(".",",")}`:"—", tarih:evdsMakro?.["TUFE_AYLIK"]?.tarih?`${evdsMakro["TUFE_AYLIK"].tarih} · canlı`:""},
                {ad:"TLREF (Gecelik Referans)", deger:evdsMakro?.["TP.BISTTLREF.KAPANIS"]?.deger!=null?`%${evdsMakro["TP.BISTTLREF.KAPANIS"].deger.toFixed(2).replace(".",",")}`:"—", tarih:evdsMakro?.["TP.BISTTLREF.KAPANIS"]?.tarih?`${evdsMakro["TP.BISTTLREF.KAPANIS"].tarih} · canlı`:""},
                (()=>{const tlrefk=tlrefkTahmini(evdsMakro); return {
                  ad:"TLREFK (Katılım)",
                  deger:tlrefk?.deger!=null?`%${tlrefk.deger.toFixed(2).replace(".",",")}`:"—",
                  tarih:tlrefk?.tarih?`${tlrefk.tarih} · TLREF-0,096`:"",
                };})(),
                {ad:"TCMB Brüt Rezerv", deger:evdsMakro?.["TP.AB.TOPLAM"]?.deger!=null?`$${(evdsMakro["TP.AB.TOPLAM"].deger/1000).toFixed(2).replace(".",",")} Mr`:"—", tarih:evdsMakro?.["TP.AB.TOPLAM"]?.tarih?`${evdsMakro["TP.AB.TOPLAM"].tarih} · canlı`:""},
              ].map((g,i,arr)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"11px 0",borderBottom:i<arr.length-1?"1px solid rgba(255,255,255,0.07)":"none"}}>
                  <div>
                    <div style={{color:"rgba(255,255,255,0.6)",fontSize:12.5,fontWeight:600}}>{g.ad}</div>
                    {g.tarih&&<div style={{color:"rgba(255,255,255,0.3)",fontSize:9.5,marginTop:1}}>{g.tarih}</div>}
                  </div>
                  <span style={{fontSize:14,fontWeight:800,fontFamily:"monospace",color:"#F1F5F9"}}>{g.deger}</span>
                </div>
              ))}
            </div>
