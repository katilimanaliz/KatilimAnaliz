...
function App(){
  ...
  //
  // GEÇİCİ DEBUG MODU: Honor tablette bildirim kaydının neden sessizce
  // başarısız olduğunu görebilmek için her adımda bir alert() gösteriyoruz.
  // Sorun çözüldükten sonra bu alert() satırları kaldırılmalı.
  useEffect(()=>{
    if(!IS_NATIVE) return;

    PushNotifications.addListener("registration", (token) => {
      console.log("Push registration token:", token.value);
      alert("BAŞARILI: Token alındı, backend'e gönderiliyor...");
      fetch(`${API_BASE}/api/bildirim?islem=kaydet`, {
        ...
        alert(`Backend cevabı: ${r.status}`);
      }).catch((e) => {
        alert("Token gönderilirken HATA: " + String(e));
    ...
    PushNotifications.addListener("registrationError", (err) => {
      ...
      alert("KAYIT HATASI (registrationError): " + JSON.stringify(err));
    });
    ...
    (async () => {
      try {
        ...
        alert("İzin durumu: " + JSON.stringify(izin));
        if (izin.receive === "prompt") {
          izin = await PushNotifications.requestPermissions();
          alert("İstek sonrası izin durumu: " + JSON.stringify(izin));
        }
        if (izin.receive !== "granted") {
          alert("İzin verilmedi, register() çağrılmıyor. Durum: " + izin.receive);
          console.log("Kullanıcı bildirim iznini reddetti");
          return;
        }
        alert("register() çağrılıyor...");
        await PushNotifications.register();
        alert("register() çağrısı tamamlandı, token bekleniyor...");
      } catch (e) {
        alert("GENEL HATA (try/catch): " + String(e));
