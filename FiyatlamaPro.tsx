...
function FinansalGostergeler({onKurTikla}:any){
  ...
  const rezerv =evdsMakro?.["TP_AB_TOPLAM"];
  ...
  const SABIT=[
    ...
    {kategori:"Referans Oranlar (BIST)",icon:"📐",color:"#8B5CF6",items:[
      ...
      {ad:"TLREFK (Katılım Bankacılığı)",
       ...
       tarih:evdsMakro?.["TP.BISTTLREFK.KAPANIS"]?.tarih||"BIST",
       canli:evdsMakro?.["TP.BISTTLREFK.KAPANIS"]!=null,
       seri:evdsMakro?.["TP.BISTTLREFK.KAPANIS_SERI"],seriAd:"TLREFK (Yıllıklandırılmış)"},
    ]},
    ...
    {kategori:"Referans Faizler",icon:"🌐",color:"#2A7A72",items:[
      ...
      {ad:"EURIBOR 6M",       deger:fmtFred(eur6m)||"—", tarih:fmtFreT(eur6m), canli:eur6m!=null,
       seri:evdsMakro?.["FRED_EUR6M_SERI"],seriAd:"EURIBOR 6M"},
    ]},
          ...
          <div style={{background:"#0F1923",padding:"12px 12px 0",paddingBottom:108,boxSizing:"border-box",overflowY:"auto"}}>
            ...
            {piyasaTabloFiltre==="fonlar"?(
              ...
              const GOSTERGELER=[
                ...
                {ad:"EURIBOR 6M",          deger:evdsMakro?.["FRED_EUR6M"]?.deger!=null?`%${evdsMakro["FRED_EUR6M"].deger.toFixed(2).replace(".",",")}`:"—", tarih:evdsMakro?.["FRED_EUR6M"]?.tarih||"FRED", canli:evdsMakro?.["FRED_EUR6M"]!=null},
                {ad:"Türkiye 5Y CDS",      deger:"~250 bps", tarih:"Haziran 2026"},
                {ad:"EMBI+ Türkiye",       deger:"~280 bps", tarih:"Haziran 2026"},
              ];
              return(
              ...
              return (
                <div key={t.tab} onClick={()=>nav(t.key)} style={{
                  ...
                  <div style={{
                    ...
                    padding:"5px 14px 6px",borderRadius:16,
                    background:aktif?"rgba(91,155,216,0.16)":"transparent",
                    transition:"background 0.2s ease",
                  }}>
                    <AltBarIcon tip={t.tip} aktif={aktif}/>
                    <span style={{fontSize:10.5,fontWeight:aktif?700:500,letterSpacing:"0.01em",color:aktif?"#5B9BD8":"rgba(255,255,255,0.45)",whiteSpace:"nowrap",transition:"color 0.2s ease"}}>{t.label}</span>
                  </div>
                </div>
              );
            })}
