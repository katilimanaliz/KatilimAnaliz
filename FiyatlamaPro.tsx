...
function FonGetiriIzleme({ settings }) {
  ...
  const [yukleniyor,  setYukleniyor]  = useState(() => {
    try {
      ...
      return !(typeof ts === "number" && (Date.now() - ts) < 5 * 60 * 1000);
    } catch { return true; }
  });
  ...
  // ── Canlı veri çek ────────────────────────────────────────────────────────
  useEffect(()=>{
    // Cache 5 dakikadan taze ise (ör. Ana Sayfa'daki widget'tan gelmiş olabilir)
    ...
    try {
      ...
      if (raw) {
        const { ts } = JSON.parse(raw);
        cacheTaze = typeof ts === "number" && (Date.now() - ts) < 5 * 60 * 1000;
      }
    } catch {}
...
function KatilimEndeksiTopHareketliler({ nav }: { nav: (sc: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (!cacheTaze("kea_hisseler")) {
      fetch("/api/hisse-proxy")
        .then(r => r.json())
        .then(d => { if (d.success) { setHisseler(d.data || []); yazCache("kea_hisseler", d.data || []); } })
        .catch(() => {});
    }
    if (!cacheTaze("kea_fonlar")) {
      fetch(`/api/tefas-proxy?t=${Date.now()}`)
        .then(r => r.json())
        .then(d => {
          if (d.success) {
  ...
  const Row = ({ item, index }: { item: any; index: number }) => {
    ...
    const up = deg >= 0;
    return (
      <div style={{ display: "flex", alignItems: "center", padding: "8px 2px", borderBottom: index === 4 ? "none" : "1px solid rgba(255,255,255,0.06)", minWidth: 0 }}>
      ...
      <div style={{paddingTop:0}}>
        ...
        {screen==="home"&&(
          <div style={{background:"#0F1923",padding:"10px 12px 0",paddingBottom:92,boxSizing:"border-box",display:"flex",flexDirection:"column",overflowY:"auto"}}>
            ...
            {/* Katılım Endeksi Top Hareketliler */}
            <KatilimEndeksiTopHareketliler nav={nav}/>
