...
function GetiridenAnapara({s}){
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      <Card>
        <Seg options={[{v:"gunluk",l:"Günlük %"},{v:"yillik",l:"Yıllık %"}]} value={tip} onChange={setTip}/>
        ...
        <Field label={`Kâr Payı Oranı (${tip==="yillik"?"Yıllık":"Günlük"})`} value={oran} onChange={setOran} suffix="%"/>
      </Card>
      {r&&<Card>
        ...
        <div style="margin:16px 24px;padding:12px 16px;background:#FFFBEB;border-left:4px solid #F59E0B;border-radius:4px;">
          <div style="display:flex;align-items:flex-start;gap:10px;">
            ...
            <p style="margin:0;font-size:10px;color:#374151;font-style:italic;line-height:1.7;">
              Bu hesaplamalar yalnızca bilgilendirme amaçlıdır; kesin teklif, resmi belge veya hukuki taahhüt niteliği taşımaz. Bu uygulama bankanızın yetkili bir birimi değildir — nihai oran ve koşullar için bankanız ile iletişime geçiniz.
            </p>
          </div>
        </div>
      `;
      ...
      {r && !r.limitAsimi && (
        <Card>
          ...
          {(()=>{
            ...
            return(<>
          <RaporButon baslik={`${tur==="soik"?"SÖİK":"Reeskont Kredisi"}${dovizAktif?` (${dovizTur})`:""} Finansman Analizi`} plan={r.plan} satirlar={raporSatirlari}/>
...
function HtForwardHesaplama(){
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      // SOFR (USD) veya EURIBOR (EUR) — /api/evds-proxy'nin FRED_SOFR / FRED_EUR3M
      // alanlarından otomatik çekiliyor (daha önce bu alan hiç doldurulmuyordu,
      // kullanıcı her seferinde elle girmek zorunda kalıyordu).
      const bazFaizDeger = bazPara==="USD" ? evdsD?.seriler?.["FRED_SOFR"]?.deger : evdsD?.seriler?.["FRED_EUR3M"]?.deger;
      if(bazFaizDeger!=null) setBazFaiz(String(bazFaizDeger).replace(".",",")); else sorunlar.push(`${bazPara==="USD"?"SOFR":"EURIBOR"} alınamadı (/api/evds-proxy)`);
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      ...
      <Card>
        ...
        <HtField label={bazPara==="USD"?"SOFR (Canlı veya elle girin)":"EURIBOR (Canlı veya elle girin)"} value={bazFaiz} onChange={setBazFaiz} suffix="%"/>
...
function HtSwapHesaplama(){
  ...
  // Bir para biriminin canlı faiz oranını getiren yardımcı — TRY için TCMB
  // AOFM, USD için SOFR, EUR için EURIBOR (3A). EUR/USD çifti eklenince artık
  // "karşı taraf her zaman TRY" varsayımı geçerli değil, bu yüzden genelleştirildi.
  const paraFaizDeger=(para:string, evdsD:any)=>{
  ...
  const paraFaizEtiket=(para:string)=>{
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      // Spot kur: TRY içeren çiftlerde doğrudan TCMB/piyasa kuru, EUR/USD gibi
      // TRY içermeyen çiftlerde ise iki çapraz TRY kurundan (EUR/TRY ÷ USD/TRY) türetiliyor.
      let kur:number|null=null;
      if(karsiPara==="TRY"){
        kur = bazPara==="USD"?kurD?.USD_TRY:kurD?.EUR_TRY;
      } else if(bazPara==="EUR"&&karsiPara==="USD"){
        kur = (kurD?.EUR_TRY&&kurD?.USD_TRY) ? kurD.EUR_TRY/kurD.USD_TRY : null;
      }
      if(kur) setSpotKur(String(kur).replace(".",","));
      const bazFaizDeger = paraFaizDeger(bazPara, evdsD);
      if(bazFaizDeger!=null) setBazFaiz(String(bazFaizDeger).replace(".",","));
      const karsiFaizDeger = paraFaizDeger(karsiPara, evdsD);
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      <Card>
        ...
        <div style={{display:"flex",gap:6,marginBottom:10}}>
          {([["USDTRY","USD/TRY"],["EURTRY","EUR/TRY"],["EURUSD","EUR/USD"]] as const).map(([v,l])=>(
            <button key={v} onClick={()=>setDovizCifti(v)} style={{
              flex:1,padding:"8px 4px",borderRadius:8,border:`1.5px solid ${dovizCifti===v?C.blue:C.border}`,
              background:dovizCifti===v?C.blueLight:C.card,color:dovizCifti===v?C.blue:C.sub,
              fontWeight:dovizCifti===v?700:500,fontSize:12,cursor:"pointer"
            }}>{l}</button>
          ))}
      ...
      <Card>
        ...
        <HtField label="Vade (Gün)" value={gun} onChange={setGun} suffix="Gün"/>
        <HtField label={`${bazPara} Faiz Oranı — ${paraFaizEtiket(bazPara)} (Canlı)`} value={bazFaiz} onChange={setBazFaiz} suffix="%"/>
        <HtField label={`${karsiPara} Faiz Oranı — ${paraFaizEtiket(karsiPara)} (Canlı)`} value={karsiFaiz} onChange={setKarsiFaiz} suffix="%"/>
      </Card>
      ...
      <div style={{background:C.card,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px 18px 32px"}}>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"12px 14px"}}>
            ...
            <p style={{margin:0,fontSize:11,color:"rgba(255,255,255,0.5)",lineHeight:1.5}}>Bu uygulamadaki hesaplamalar bilgilendirme amaçlıdır; kesin teklif, resmi belge veya hukuki taahhüt niteliği taşımaz ve hukuki sonuç doğurmaz. Bu uygulama bankanızın yetkili bir birimi değildir — nihai oran ve koşullar için bankanız ile iletişime geçiniz.</p>
          </div>
