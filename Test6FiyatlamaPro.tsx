...
function SoikReeskontHesaplama({s,onGecmis}:any){
  ...
  const r = useCallback(()=>{
    ...
    // Azalan bakiye üzerinden taksit taksit kâr payı + BSMV hesabı (yıllık oran, 360 gün baz)
    let kalan = T;
    ...
    const efektifOran = netKullandirilan > 0
      ? ((toplamKarPayi + toplamBsmv + bankaKomisyonu) / netKullandirilan) * (360/gun) * 100
      : yillikOran;
      ...
      {r && (
        <Card>
          ...
          <div style={{marginTop:8,padding:"12px 14px",background:"#FFF4E5",borderRadius:12,border:`1.5px solid ${C.orange}`}}>
            <p style={{margin:"0 0 4px",fontSize:11,fontWeight:700,color:C.sub,letterSpacing:"0.04em"}}>EFEKTİF YILLIK MALİYET</p>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
              <p style={{margin:0,fontSize:12,color:C.sub}}>Yıllık %{fmtN(r.oran)} → Efektif (komisyon dahil)</p>
            ...
            <p style={{margin:"6px 0 0",fontSize:10,color:C.sub,lineHeight:1.5}}>
              Kâr payı azalan anapara bakiyesi üzerinden taksit taksit hesaplanır. BSMV kâr payı üzerinden ayrıca tahakkuk eder. Banka komisyonu vade başında peşin tahsil edilir.
            </p>
          </div>
          ...
          <RaporButon baslik={`${tur==="soik"?"SÖİK":"Reeskont Kredisi"} Finansman Analizi`} plan={r.plan} satirlar={[
            ...
            {label:"Toplam Geri Ödeme", value:fmtTL(r.geriOdenecekAnapara)},
