...
// ─── HAZİNE İŞLEMLERİ ─────────────────────────────────────────────────────────
const HT_PARA_BIRIMLERI = ["TRY","USD","EUR","SAR","RUB","XAU","XAG"];
const HT_PARA_ETIKET:any = {TRY:"TRY",USD:"USD",EUR:"EUR",SAR:"SAR",RUB:"RUB",XAU:"Altın (gr)",XAG:"Gümüş (gr)"};
...
function HtDovizDonusturucu(){
  ...
  useEffect(()=>{
    fetch("/api/kur").then(r=>r.ok?r.json():null).then(d=>{
      if(d){
        setKurlar({
          USD: d.USD_TRY, EUR: d.EUR_TRY, SAR: d.SAR_TRY, RUB: d.RUB_TRY,
  ...
  return(
    <div style={{padding:"0 16px 32px"}}>
      ...
      <Card>
        ...
        <HtField label={`${bazPara} Faiz Oranı — SOFR/EURIBOR (elle girin)`} value={bazFaiz} onChange={setBazFaiz} suffix="%"/>
        ...
        {!yukleniyor && hataDetay.length>0 ? (
          <div style={{background:"#FDEDED",borderRadius:8,padding:"8px 10px",marginTop:4}}>
