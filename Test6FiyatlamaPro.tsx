...
function SoikReeskontHesaplama({s,onGecmis}:any){
  ...
  const yillikOran = (s as any)[seciliVade.oranKey] ?? 0;
  ...
  const r = useCallback(()=>{
    ...
    const bankaKomisyonu = T * (SOIK_REESKONT_KOMISYON_ORAN/100); // peşin tahsil edilir, taksit planına dahil değil
    let kalan = T;
    ...
    // Efektif maliyet: toplam ödenen maliyet (kâr+BSMV+komisyon) / başlangıç anaparası
    // Not: çok taksitli planlarda anapara azalan bakiye üzerinden ödendiği için bu oran,
    // basit yıllık orandan düşük çıkabilir — bu beklenen bir durumdur (ortalama kullanım T'den az).
    const efektifOran = T > 0
      ? ((toplamKarPayi + toplamBsmv + bankaKomisyonu) / T) * 100
      : yillikOran;

    return {
      tur, toplamKarPayi, toplamBsmv, bankaKomisyonu, netKullandirilan,
      ...
      pesinKesinti:false, limitAsimi: T > REESKONT_GUNLUK_LIMIT_TL,
    };
  },[tutar,seciliVade,yillikOran,tur,bsmvMuaf])();
    ...
    <div style={{padding:"0 16px 32px"}}>
      ...
      <Card>
            ...
            <button key={v.id} onClick={()=>setVadeId(v.id)} style={{
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <span style={{fontSize:13,fontWeight:700,color:vadeId===v.id?C.blue:C.sub}}>
                  %{((s as any)[v.oranKey] ?? 0).toLocaleString("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2})} Yıllık
                </span>
              </div>
        ...
        {tur==="reeskont" && (seciliVade as any).savunmaSanayiSarti && (
          <div style={{marginTop:8,padding:"10px 14px",borderRadius:10,background:"#FDEDED",border:`1.5px solid ${C.red}`}}>
        ...
        <div style={{
          ...
          <span style={{fontSize:14,fontWeight:800,color:C.purple,whiteSpace:"nowrap",marginLeft:8}}>
            {tur==="reeskont" ? `%${fmtN(Math.min(REESKONT_KOMISYON_ORAN_YILLIK*(seciliVade.gun/360),REESKONT_KOMISYON_ORAN_YILLIK),4)}` : `%${SOIK_REESKONT_KOMISYON_ORAN}`}
          </span>
        </div>
        ...
        {tur==="reeskont" && (
          <div style={{marginTop:10,padding:"10px 14px",borderRadius:10,background:"#F5F6F8",border:`1.5px solid ${C.border}`}}>
      ...
      {r && r.limitAsimi && (
        <div style={{margin:"0 0 12px",padding:"12px 14px",background:"#FDEDED",borderRadius:12,border:`1.5px solid ${C.red}`}}>
          <p style={{margin:0,fontSize:12,fontWeight:700,color:C.red}}>⚠️ Firma Bazlı Günlük Kullanım Limiti Aşıldı</p>
      ...
      {r && (
        <Card>
          ...
          <RRow label="Finansman Tutarı (Anapara)" value={fmtTL(parseFloat(tutar)||0)}/>
          <RRow label="Vade" value={`${r.gun} Gün, ${r.taksit} Taksit`}/>
          <RRow label="Yıllık Kâr Oranı (Basit)" value={`%${fmtN(r.oran)}`}/>
          {r.pesinKesinti ? (
            <>
              <RRow label="Kâr Payı (Vade Başında Peşin Kesilir)" value={fmtTL(r.toplamKarPayi)} accent={C.orange}/>
              <RRow label={`Banka Komisyonu (%${fmtN(r.komisyonFiiliOran,4)} — Net Bakiyeden Peşin)`} value={fmtTL(r.bankaKomisyonu)} accent={C.purple} sub/>
              <RRow label="Müşteri Hesabına Geçen Net Tutar" value={fmtTL(r.netKullandirilan)} accent={C.blue} big/>
              <RRow label="Vade Sonu Geri Ödeme (Nominal Anapara)" value={fmtTL(r.geriOdenecekAnapara)} accent={C.green} big/>
            </>
          ) : (
            <>
              ...
              <RRow label={bsmvMuaf?"Toplam BSMV (Muaf)":"Toplam BSMV (%5)"} value={fmtTL(r.toplamBsmv)} accent={C.orange} sub/>
              <RRow label="Toplam Geri Ödeme (Anapara+Kâr+BSMV)" value={fmtTL(r.geriOdenecekAnapara)} accent={C.green} big/>
              <RRow label={`Ortalama Taksit Tutarı (${r.taksit} taksit)`} value={fmtTL(r.taksitTutari)} sub/>
            </>
          )}
          ...
          <RaporButon baslik={`${tur==="soik"?"SÖİK":"Reeskont Kredisi"} Finansman Analizi`} plan={r.plan} satirlar={r.pesinKesinti ? [
            {label:"Finansman Tutarı (Anapara)", value:fmtTL(parseFloat(tutar)), big:true},
            {label:"Vade", value:`${r.gun} gün, tek taksit`},
            {label:"Yıllık Kâr Oranı (Basit)", value:`% ${fmtN(r.oran)}`},
            {label:"Kâr Payı (Vade Başında Peşin)", value:fmtTL(r.toplamKarPayi)},
            {label:`Banka Komisyonu (%${fmtN(r.komisyonFiiliOran,4)} — Net Bakiyeden)`, value:fmtTL(r.bankaKomisyonu)},
            {label:"Müşteri Hesabına Geçen Net Tutar", value:fmtTL(r.netKullandirilan), big:true},
            {label:"Vade Sonu Geri Ödeme (Nominal)", value:fmtTL(r.geriOdenecekAnapara), big:true},
            ...
            {label:"Toplam Kâr Payı", value:fmtTL(r.toplamKarPayi)},
            {label:"Toplam BSMV", value:fmtTL(r.toplamBsmv)},
            {label:"Toplam Geri Ödeme", value:fmtTL(r.geriOdenecekAnapara), big:true},
            {label:`Ortalama Taksit Tutarı`, value:fmtTL(r.taksitTutari)},
            {label:"Efektif Yıllık Maliyet", value:`% ${fmtN(r.efektifOran,2)}`, big:true},
          ]}/>
...
function PiyasaHaberleri(){
  ...
  const [yukleniyor,setYukleniyor]=useState(true);
  const [hata,setHata]=useState<string|null>(null);
  const [etkiFiltre,setEtkiFiltre]=useState<"tumu"|"High"|"Medium">("tumu");
  ...
  const formatTarihSaat=(iso:string)=>{
    ...
    return {gun:`${DAYS[d.getDay()]} ${d.getDate()} ${MONTHS[d.getMonth()]}`, saat, tarihObj:d};
  };

  const filtreli = etkiFiltre==="tumu" ? haberler : haberler.filter(h=>h.etki===etkiFiltre);
  ...
  return(
    <div style={{background:"#0B0E14",minHeight:"100vh",paddingBottom:32}}>
      <div style={{background:"linear-gradient(135deg,#1A1A2E 0%,#16213E 100%)",padding:"16px 16px 14px",borderBottom:"2px solid #FF6B35"}}>
        ...
        <p style={{margin:"4px 0 0",fontSize:11,color:"rgba(255,255,255,0.55)"}}>
          Bu haftanın önemli finansal olayları {guncelleme && `· Güncelleme: ${new Date(guncelleme).toLocaleTimeString("tr-TR",{hour:"2-digit",minute:"2-digit"})}`}
        </p>
      </div>
      ...
      <div style={{display:"flex",gap:6,padding:"12px 16px 0"}}>
        {[
          ...
          {v:"High" as const,l:"🔴 Yüksek Etki"},
          {v:"Medium" as const,l:"🟠 Orta Etki"},
        ].map(f=>(
          <button key={f.v} onClick={()=>setEtkiFiltre(f.v)} style={{
            flex:1,padding:"8px 6px",borderRadius:8,border:`1.5px solid ${etkiFiltre===f.v?"#FF6B35":"#2A2F3E"}`,
            background:etkiFiltre===f.v?"rgba(255,107,53,0.15)":"transparent",
            color:etkiFiltre===f.v?"#FF6B35":"#9CA3AF",
            fontWeight:etkiFiltre===f.v?700:500,fontSize:11,cursor:"pointer"
          }}>{f.l}</button>
        ))}
      </div>
      ...
      <div style={{padding:"14px 16px"}}>
        {yukleniyor && (
          <div style={{textAlign:"center",padding:40,color:"#9CA3AF",fontSize:13}}>⏳ Piyasa verileri yükleniyor...</div>
        )}
        {!yukleniyor && hata && (
          <div style={{textAlign:"center",padding:40,color:"#F87171",fontSize:13}}>⚠️ {hata}</div>
        )}
        {!yukleniyor && !hata && filtreli.length===0 && (
          <div style={{textAlign:"center",padding:40,color:"#9CA3AF",fontSize:13}}>Bu hafta için kayıt bulunamadı</div>
        )}
      ...
      <div style={{margin:"0 16px",padding:"10px 14px",background:"#151823",borderRadius:10}}>
        <p style={{margin:0,fontSize:10,color:"#6B7280",lineHeight:1.5}}>
          📡 Veriler Forex Factory ekonomik takviminden otomatik çekilmektedir. Sadece yüksek ve orta etkili olaylar gösterilmektedir. Yatırım tavsiyesi değildir.
        </p>
      </div>
    </div>
  );
...
{/* ── HESAPLAMA ARAÇLARI ── */}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:10}}>
              {[
                ...
                {key:"piyasaHaberleri",    icon:"📡", label:"Piyasa\nHaberleri",  g1:"#1A1A2E",g2:"#16213E"},
              ].map((c,i)=>(
                <div key={i} onClick={()=>nav(c.key)} style={{
                  background:`linear-gradient(145deg,${c.g1} 0%,${c.g2} 100%)`,
                  borderRadius:14,cursor:"pointer",padding:"12px 8px",
...
{/* ── KATILIM MENU ── */}
        ...
        {/* ── YASAL UYARI FOOTER ── */}
        {!["home","katilimMenu","finansmanMenu","ticariMenu","asistan","sozluk","finansalTakvim","finansalGostergeler","oranKarsilastirma","vadeTakibi","hazineIslemleri","piyasaHaberleri","ayarlar"].includes(screen)&&(
          <div style={{
