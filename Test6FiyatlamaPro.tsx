...
function FonGetiriIzleme({ settings }) {
  ...
  const siralanmis = useMemo(()=>{
    const dir = sirala?.endsWith("D") ? -1 : 1;
    ...
    return [...(fonlar)].sort((a,b)=>{
      const av = (a as any)[field] ?? (dir===1?-Infinity:Infinity);
      const bv = (b as any)[field] ?? (dir===1?-Infinity:Infinity);
      return (bv - av) * dir;
    });
  },[fonlar, sirala]);
                ...
                <div key={fon.kod}>
                  ...
                  <div onClick={()=>handleFonTikla(fon)} style={{
                    ...
                    <div style={{width:38,flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-start",gap:1}}>
                      ...
                      {fon.katilimUygun&&<span style={{fontSize:7,background:"#166534",color:"#86efac",borderRadius:3,padding:"0px 3px",fontWeight:700,lineHeight:"12px"}}>KAT</span>}
                    </div>
                    <div style={{flex:1,minWidth:0,paddingRight:2,textAlign:"left"}}>
                      <div style={{fontSize:11,fontWeight:600,color:"#cdd9e5",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",lineHeight:1.3}}>{fon.ad}</div>
                      <div style={{fontSize:9,color:FC.sub2,marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{kisaYon(fon.yonetici)}</div>
                    </div>
                    ...
                    <div style={s.detayWrap}>
                      ...
                      <div style={{background:"#1a2535",borderRadius:10,padding:"10px 12px",marginBottom:10,border:`1px solid ${FC.border}`}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
                          ...
                          {fon.katilimUygun&&<span style={{fontSize:9,background:"#166534",color:"#86efac",borderRadius:4,padding:"1px 6px",fontWeight:700}}>✓ Katılım Endeksi Uygun</span>}
                        </div>
                        <div style={{fontSize:13,fontWeight:700,color:"#cdd9e5",lineHeight:1.4}}>{fon.ad}</div>
                        <div style={{fontSize:11,color:FC.sub2,marginTop:4}}>{fon.yonetici}</div>
                      </div>
                      {/* Dönem grid */}
...
function HisseDetay({ hisse, onGeri }: { hisse: any, onGeri: () => void }) {
  ...
  const SVGGrafik = () => {
    ...
    const hasOHLC = grafik[0]?.acilis != null;
    const w = 320, h = 130, pad = 4;
    const prices = grafik.flatMap(p => hasOHLC ? [p.yuksek||p.kapanis, p.dusuk||p.kapanis] : [p.kapanis]);
    const min = Math.min(...prices) * 0.999;
    const max = Math.max(...prices) * 1.001;
    const scaleY = (v: number) => h - ((v - min) / (max - min)) * h;
    const candleW = Math.max(2, Math.min(8, (w - pad*2) / grafik.length - 1));
      ...
      <div style={{position:"relative"}}>
        ...
        {tooltip && (
          <div style={{
            ...
            {hasOHLC && <><span>A:{tooltip.p.acilis?.toLocaleString("tr-TR")}</span><span>Y:{tooltip.p.yuksek?.toLocaleString("tr-TR")}</span><span>D:{tooltip.p.dusuk?.toLocaleString("tr-TR")}</span></>}
        ...
        >
          ...
          {grafik.map((p, i) => {
            const x = pad + i * stepX;
            const kap = scaleY(p.kapanis);
            const aci = hasOHLC && p.acilis ? scaleY(p.acilis) : kap;
            const yuk = hasOHLC && p.yuksek ? scaleY(p.yuksek) : kap;
            const dus = hasOHLC && p.dusuk  ? scaleY(p.dusuk)  : kap;
            const yukari = p.kapanis >= (p.acilis || p.kapanis);
            const renk3 = yukari ? "#4ade80" : "#f87171";
            const bodyTop = Math.min(kap, aci);
            const bodyH = Math.max(1, Math.abs(kap - aci));
            const isTooltip = tooltip?.p.tarih === p.tarih;

            return (
              <g key={i}>
                {/* Fitil üst */}
                <line x1={x} x2={x} y1={yuk} y2={bodyTop} stroke={renk3} strokeWidth="1"/>
                {/* Fitil alt */}
                <line x1={x} x2={x} y1={bodyTop+bodyH} y2={dus} stroke={renk3} strokeWidth="1"/>
                {/* Gövde */}
                <rect x={x - candleW/2} y={bodyTop} width={candleW} height={bodyH}
                  fill={yukari ? renk3 : renk3} stroke={renk3} strokeWidth="0.5"
                  fillOpacity={yukari ? 0.9 : 1}
                />
                {/* Seçili mum vurgusu */}
                {isTooltip && <line x1={x} x2={x} y1={0} y2={h} stroke="#ffffff33" strokeWidth="1" strokeDasharray="2,2"/>}
              </g>
            );
          })}
        </svg>
    ...
    <div style={{background:C.bg, padding:"12px 14px 80px", minHeight:"100%"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontSize:15,fontWeight:700,color:C.text}}>BIST Veri İzleme</div>
              ...
              <div style={{background:C.card,borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden"}}>
                {liste.map((h,i)=>(
                  <div key={h.ticker} onClick={()=>setDetayHisse(h)}
                    ...
                    <span style={{width:50,fontSize:12,fontWeight:800,color:yukDusAcik==="yuk"?C.green:C.red,flexShrink:0}}>{h.ticker}</span>
                    <span style={{flex:1,fontSize:11,color:C.sub,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.sirket||""}</span>
      ...
      {yukleniyor ? (
        ...
        <div>
          {/* Tablo başlığı */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 48px 44px 44px 40px 44px",gap:3,padding:"0 2px 6px",fontSize:9,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:0.5,borderBottom:`1.5px solid ${C.border}`,marginBottom:2}}>
            <span>Hisse</span>
            <span style={{textAlign:"right"}}>Fiyat</span>
            <span style={{textAlign:"right"}}>Gün%</span>
            <span style={{textAlign:"right"}}>Ay%</span>
            <span style={{textAlign:"right"}}>F/K</span>
          ...
          {filtreli.map((h, i) => (
            <div key={h.ticker}>
              <div
                ...
                style={{
                  display:"grid",gridTemplateColumns:"1fr 48px 44px 44px 40px 44px",gap:3,
                  padding:"6px 2px",
                  background: secilen?.ticker===h.ticker ? C.blueLight : i%2===0 ? C.card : C.bg,
                  borderLeft: secilen?.ticker===h.ticker ? `3px solid ${C.blue}` :
                               h.katilimEndeksi ? `3px solid ${C.green}44` : "3px solid transparent",
                  cursor:"pointer",borderRadius:6,marginBottom:1,
                }}>
                <div style={{minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:5}}>
                    <span style={{fontSize:12,fontWeight:800,color:h.katilimEndeksi?C.green:C.blue}}>{h.ticker}</span>
                    {h.katilimEndeksi && <span style={{fontSize:9,color:C.green,opacity:0.7}}>☪</span>}
                  </div>
                  <div style={{fontSize:10,color:C.sub,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:180}}>
                    {SEKTOR_TR[h.sektor] || h.sektor}
                  </div>
                </div>
                <div style={{textAlign:"right",fontSize:11,fontWeight:600,color:C.text,fontVariantNumeric:"tabular-nums"}}>{h.fiyat ? h.fiyat.toLocaleString("tr-TR",{minimumFractionDigits:2,maximumFractionDigits:2}) : "—"}</div>
                <div style={{textAlign:"right",fontSize:11,fontWeight:600,color:h.degisim1g>0?C.green:h.degisim1g<0?C.red:C.sub}}>{h.degisim1g ? (h.degisim1g>0?"+":"")+h.degisim1g.toFixed(2)+"%" : "—"}</div>
                <div style={{textAlign:"right",fontSize:11,fontWeight:600,color:h.degisim1a&&h.degisim1a>0?C.green:h.degisim1a&&h.degisim1a<0?C.red:C.sub}}>{h.degisim1a ? (h.degisim1a>0?"+":"")+h.degisim1a.toFixed(2)+"%" : "—"}</div>
                <div style={{textAlign:"right",fontSize:11,fontWeight:600,color:C.text}}>{fmt2(h.fk,1)}</div>
                <div style={{textAlign:"right",fontSize:11,fontWeight:600,color:h.roe&&h.roe>0?C.green:h.roe&&h.roe<0?C.red:C.text}}>{fmt2(h.roe,1)}</div>
              </div>
...
// ─── ORAN KARŞILAŞTIRMA ─────────────────────────────────────────────────────
// TCMB EVDS - Haftalık Ağırlıklı Ortalama Kredi/Mevduat/Katılma Oranları
// Grup bazlı: Kamu / Özel / Yabancı / Katılım

const EVDS_GRUPLARI = [
  {
    id:"tum", label:"Tüm Bankalar", icon:"🏦", renk:"#2C5F8A", bg:"#EBF3FB",
    seriler:{
      konut:"TP.KTF10", tasit:"TP.KTF11", ihtiyac:"TP.KTF12",
      mevduat:"TP.TL.MT3", katilma:null,
    }
  },
  {
    id:"kamu", label:"Kamu Bankaları", icon:"🏛️", renk:"#1565C0", bg:"#E3F2FD",
    seriler:{
      konut:"TP.KTF101", tasit:"TP.KTF111", ihtiyac:"TP.KTF121",
      mevduat:"TP.TL.MT3.K", katilma:null,
    }
  },
  {
    id:"ozel", label:"Özel Bankalar", icon:"🏢", renk:"#3A7D5C", bg:"#EBF5F0",
    seriler:{
      konut:"TP.KTF102", tasit:"TP.KTF112", ihtiyac:"TP.KTF122",
      mevduat:"TP.TL.MT3.O", katilma:null,
    }
  },
  {
    id:"yabanci", label:"Yabancı Bankalar", icon:"🌍", renk:"#5B4A8A", bg:"#F0EDF8",
    seriler:{
      konut:"TP.KTF103", tasit:"TP.KTF113", ihtiyac:"TP.KTF123",
      mevduat:null, katilma:null,
    }
  },
  {
    id:"katilim", label:"Katılım Bankaları", icon:"☪️", renk:"#B07D2E", bg:"#FBF5E8",
    seriler:{
      konut:"TP.KTF17", tasit:"TP.KTF171", ihtiyac:"TP.KTF172",
      mevduat:null, katilma:"TP.TL.MT3.KT",
    }
  },
];

const EVDS_URUNLER:{[k:string]:{label:string,icon:string,renk:string}} = {
  konut:   {label:"Konut Fin.",    icon:"🏠", renk:"#2C5F8A"},
  tasit:   {label:"Taşıt Fin.",   icon:"🚗", renk:"#2A7A72"},
  ihtiyac: {label:"İhtiyaç Fin.", icon:"💼", renk:"#5B4A8A"},
  mevduat: {label:"Mevduat",      icon:"🏦", renk:"#3A7D5C"},
  katilma: {label:"Katılma Hes.", icon:"☪️", renk:"#B07D2E"},
};

// Grafik Modal - EVDS trend grafiği
function OranGrafikModal({grup,urun,seriKodu,onClose}:any){
  const [grafik,setGrafik]=useState<any[]>([]);
  const [yukleniyor,setYukleniyor]=useState(true);
  const [tooltip,setTooltip]=useState<any>(null);
  const urunInfo=EVDS_URUNLER[urun];
  const grupInfo=EVDS_GRUPLARI.find(g=>g.id===grup.id);

  useEffect(()=>{
    const cek=async()=>{
      try{
        const r=await fetch(`/api/evds-proxy?grafik=1&seri=${seriKodu}&t=${Date.now()}`);
        if(!r.ok) throw new Error();
        const json=await r.json();
        const tarihsel=json.tarihsel?.[seriKodu]||[];
        setGrafik(tarihsel.filter((p:any)=>p.deger!==null));
      }catch{setGrafik([]);}
      setYukleniyor(false);
    };
    cek();
  ...
  // SVG grafik hesaplamaları
  const W=320, H=130, PAD={l:36,r:10,t:12,b:28};
  const gW=W-PAD.l-PAD.r, gH=H-PAD.t-PAD.b;
  const vals=grafik.map(p=>p.deger);
  const minV=vals.length?Math.min(...vals):0;
  const maxV=vals.length?Math.max(...vals):100;
  const span=maxV-minV||1;
  const px=(i:number)=>PAD.l+i*(gW/(Math.max(grafik.length-1,1)));
  const py=(v:number)=>PAD.t+gH-(((v-minV)/span)*gH);

  const polyline=grafik.map((p,i)=>`${px(i)},${py(p.deger)}`).join(" ");
  const area=grafik.length>1?`M${px(0)},${py(grafik[0].deger)} `+
    grafik.slice(1).map((p,i)=>`L${px(i+1)},${py(p.deger)}`).join(" ")+
    ` L${px(grafik.length-1)},${PAD.t+gH} L${px(0)},${PAD.t+gH} Z`:"";

  // Tarih formatlama: "13-06-2025" → "13 Haz"
  const AYLAR_K=["Oca","Şub","Mar","Nis","May","Haz","Tem","Ağu","Eyl","Eki","Kas","Ara"];
  const fmtTarihKisa=(t:string)=>{
    if(!t) return "";
    const [g,m]=t.split("-");
    return `${g} ${AYLAR_K[parseInt(m)-1]||m}`;
  };
  const fmtTarihUzun=(t:string)=>{
    if(!t) return "";
    const [g,m,y]=t.split("-");
    const AYLAR_U=["Ocak","Şubat","Mart","Nisan","Mayıs","Haziran","Temmuz","Ağustos","Eylül","Ekim","Kasım","Aralık"];
    return `${parseInt(g)} ${AYLAR_U[parseInt(m)-1]||m} ${y}`;
  };

  const sonVeri=grafik[grafik.length-1];

  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:200,display:"flex",alignItems:"flex-end"}}
      onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"80vh",overflowY:"auto",padding:"0 0 32px"}}>
        {/* Handle */}
        <div style={{display:"flex",justifyContent:"center",padding:"10px 0 4px"}}>
          <div style={{width:36,height:4,borderRadius:2,background:"#DDE3EA"}}/>
        </div>
        {/* Başlık */}
        <div style={{padding:"8px 20px 12px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div>
              <p style={{margin:0,fontSize:11,color:C.sub,fontWeight:600}}>
                {grupInfo?.icon} {grupInfo?.label}
              </p>
              <h3 style={{margin:"2px 0 0",fontSize:17,fontWeight:800,color:C.label}}>
                {urunInfo.icon} {urunInfo.label}
              </h3>
              {sonVeri&&(
                <p style={{margin:"3px 0 0",fontSize:11,color:C.sub}}>
                  Son veri: <b style={{color:grupInfo?.renk}}>{fmtTarihUzun(sonVeri.tarih)} haftası</b>
                </p>
              )}
            </div>
            <button onClick={onClose} style={{border:"none",background:"#F0F4F8",borderRadius:20,width:32,height:32,fontSize:16,cursor:"pointer",color:C.sub}}>✕</button>
          </div>
          {sonVeri&&(
            <div style={{marginTop:10,background:grupInfo?.bg||C.blueLight,borderRadius:10,padding:"8px 14px",display:"inline-flex",alignItems:"baseline",gap:6}}>
              <span style={{fontSize:26,fontWeight:800,color:grupInfo?.renk||C.blue}}>
                %{sonVeri.deger.toFixed(2).replace(".",",")}
              </span>
              {grafik.length>=2&&(()=>{
                const diff=sonVeri.deger-grafik[grafik.length-2].deger;
                const isPos=diff>0;
                const isFon=urun==="mevduat"||urun==="katilma";
                const isIyi=isFon?isPos:!isPos;
                return(
                  <span style={{fontSize:13,fontWeight:700,color:isIyi?"#3A7D5C":"#B83232"}}>
                    {isPos?"▲":"▼"} {Math.abs(diff).toFixed(2).replace(".",",")} bp
                  </span>
                );
              })()}
            </div>
        ...
        <div style={{padding:"16px 16px 8px"}}>
          <p style={{margin:"0 0 8px",fontSize:11,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:"0.05em"}}>
            Son {grafik.length} Hafta Trendi
          </p>
          {yukleniyor?(
              ...
              <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{display:"block",overflow:"visible"}}>
                <defs>
                  <linearGradient id="okGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={grupInfo?.renk||C.blue} stopOpacity="0.25"/>
                    <stop offset="100%" stopColor={grupInfo?.renk||C.blue} stopOpacity="0.02"/>
                  </linearGradient>
                </defs>
                {/* Grid yatay */}
                {[0,0.25,0.5,0.75,1].map(t=>{
                  const yy=PAD.t+t*gH;
                  const val=maxV-(t*span);
                  return(
                    <g key={t}>
                      <line x1={PAD.l} y1={yy} x2={W-PAD.r} y2={yy} stroke="#EEF1F5" strokeWidth="1"/>
                      <text x={PAD.l-4} y={yy+4} textAnchor="end" fontSize="8" fill="#9CA3AF">
                        {val.toFixed(1)}
                      </text>
                    </g>
                  );
                ...
                {/* Alan */}
                {area&&<path d={area} fill="url(#okGrad)"/>}
                {/* Çizgi */}
                <polyline points={polyline} fill="none" stroke={grupInfo?.renk||C.blue} strokeWidth="2" strokeLinejoin="round"/>
                {/* Noktalar + dokunma alanı */}
                {grafik.map((p,i)=>{
                  const cx=px(i), cy=py(p.deger);
                  const isActive=tooltip?.i===i;
                  return(
                    <g key={i}
                      onMouseEnter={()=>setTooltip({i,p,cx,cy})}
                      onTouchStart={()=>setTooltip({i,p,cx,cy})}>
                      <circle cx={cx} cy={cy} r={isActive?5:3}
                        fill={isActive?(grupInfo?.renk||C.blue):"#fff"}
                        stroke={grupInfo?.renk||C.blue} strokeWidth="2"/>
                      {/* geniş dokunma alanı */}
                      <rect x={cx-12} y={PAD.t} width={24} height={gH} fill="transparent"/>
                    </g>
                  );
                })}
                {/* X ekseni tarih etiketleri — ilk, orta, son */}
                {[0,Math.floor((grafik.length-1)/2),grafik.length-1].filter((v,i,a)=>a.indexOf(v)===i).map(i=>(
                  <text key={i} x={px(i)} y={H-6} textAnchor="middle" fontSize="8" fill="#9CA3AF">
                    {fmtTarihKisa(grafik[i].tarih)}
                  </text>
                ))}
                {/* Tooltip dikey çizgi */}
                {tooltip&&(
                  <line x1={tooltip.cx} y1={PAD.t} x2={tooltip.cx} y2={PAD.t+gH}
                    stroke={grupInfo?.renk||C.blue} strokeWidth="1" strokeDasharray="3,3" opacity="0.5"/>
                )}
              </svg>
              {/* Tooltip kutusu */}
              {tooltip&&(
                <div style={{
                  position:"absolute",
                  top:Math.max(PAD.t,tooltip.cy-52),
                  left:tooltip.cx>W*0.6?"auto":"50%",
                  right:tooltip.cx>W*0.6?"4px":"auto",
                  transform:tooltip.cx<=W*0.6?"translateX(-50%)":"none",
                  background:"#1C2B3A",color:"#fff",borderRadius:10,
                  padding:"6px 12px",pointerEvents:"none",zIndex:10,whiteSpace:"nowrap",
                }}>
                  <p style={{margin:0,fontSize:10,opacity:0.7}}>{fmtTarihUzun(tooltip.p.tarih)} haftası</p>
                  <p style={{margin:"2px 0 0",fontSize:15,fontWeight:800}}>
                    %{tooltip.p.deger.toFixed(2).replace(".",",")}
                  </p>
                </div>
              )}
            </div>
          )}
          {/* X ekseni başlangıç-bitiş */}
          {grafik.length>=2&&(
            <div style={{display:"flex",justifyContent:"space-between",marginTop:2}}>
              <span style={{fontSize:10,color:C.sub}}>{fmtTarihUzun(grafik[0].tarih)}</span>
              <span style={{fontSize:10,color:C.sub}}>{fmtTarihUzun(grafik[grafik.length-1].tarih)}</span>
            </div>
          )}
        </div>

        {/* Tablo - haftalık değerler */}
        {grafik.length>0&&(
          <div style={{padding:"0 16px"}}>
            <p style={{margin:"8px 0 6px",fontSize:11,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:"0.05em"}}>Haftalık Değerler</p>
            <div style={{background:"#F8FAFC",borderRadius:10,overflow:"hidden",border:`1px solid ${C.border}`}}>
              {[...grafik].reverse().map((p,i)=>{
                const prev=grafik[grafik.length-2-i];
                const diff=prev?p.deger-prev.deger:null;
                const isFon=urun==="mevduat"||urun==="katilma";
                const isIyi=diff!==null&&(isFon?diff>0:diff<0);
                return(
                  <div key={i} style={{
                    display:"flex",justifyContent:"space-between",alignItems:"center",
                    padding:"7px 12px",borderBottom:i<grafik.length-1?`1px solid ${C.border}`:"none",
                    background:i===0?`${grupInfo?.bg}`:"transparent",
                  }}>
                    <div>
                      <span style={{fontSize:12,fontWeight:i===0?700:500,color:i===0?C.label:C.sub}}>
                        {fmtTarihUzun(p.tarih)} haftası
                      </span>
                      {i===0&&<span style={{marginLeft:6,fontSize:10,background:grupInfo?.renk,color:"#fff",borderRadius:6,padding:"1px 6px",fontWeight:700}}>Son</span>}
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      {diff!==null&&(
                        <span style={{fontSize:10,fontWeight:600,color:isIyi?"#3A7D5C":"#B83232"}}>
                          {diff>0?"+":""}{diff.toFixed(2).replace(".",",")}
                        </span>
                      )}
                      <span style={{fontSize:14,fontWeight:800,color:i===0?(grupInfo?.renk||C.blue):C.label}}>
                        %{p.deger.toFixed(2).replace(".",",")}
                      </span>
                    </div>
                  </div>
                );
        ...
        <div style={{padding:"10px 16px 0"}}>
          <p style={{margin:0,fontSize:10,color:"#CBD5E1"}}>EVDS Seri: <b>{seriKodu}</b> · TCMB Haftalık Ağırlıklı Ortalama</p>
        </div>
      </div>
    </div>
  );
...
function OranKarsilastirma(){
  const [veriler,setVeriler]=useState<any>(null);
  const [yukleniyor,setYukleniyor]=useState(true);
  const [hata,setHata]=useState<string|null>(null);
  const [siralamaSutun,setSiralamaSutun]=useState<string|null>(null);
  const [siralamaAsc,setSiralamaAsc]=useState(true);
  const [veriTarihi,setVeriTarihi]=useState<string|null>(null); // EVDS'den gelen hafta tarihi
  const [grafik,setGrafik]=useState<{grup:any,urun:string,seriKodu:string}|null>(null);
  ...
  const verileriCek=useCallback(async()=>{
    ...
    try{
      ...
      // EVDS'den gelen gerçek veri tarihi (TP.KTF10'un tarihi referans alınır)
      const refTarih=json.seriler?.["TP.KTF10"]?.tarih;
      if(refTarih){
        // "13-06-2025" → "13 Haziran 2025 haftası"
        const [g,m,y]=refTarih.split("-");
        const AYLAR=["Ocak","Şubat","Mart","Nisan","Mayıs","Haziran","Temmuz","Ağustos","Eylül","Ekim","Kasım","Aralık"];
        setVeriTarihi(`${parseInt(g)} ${AYLAR[parseInt(m)-1]||m} ${y} haftası`);
      }
    }catch(e:any){
  ...
  const getSeriVeri=(seriKodu:string|null)=>{
    if(!seriKodu||!veriler) return {deger:null,tarih:null};
    const v=veriler[seriKodu];
    return {deger:v?.deger??null, tarih:v?.tarih??null};
  };

  const handleSirala=(sutun:string)=>{
    if(siralamaSutun===sutun) setSiralamaAsc(!siralamaAsc);
    else{setSiralamaSutun(sutun);setSiralamaAsc(true);}
  };

  const siraliGruplar=[...EVDS_GRUPLARI].sort((a,b)=>{
    if(!siralamaSutun) return 0;
    const av=getSeriVeri(a.seriler[siralamaSutun as keyof typeof a.seriler]).deger??9999;
    const bv=getSeriVeri(b.seriler[siralamaSutun as keyof typeof b.seriler]).deger??9999;
    return siralamaAsc ? av-bv : bv-av;
  });

  const stats:any={};
  Object.keys(EVDS_URUNLER).forEach(urun=>{
    const vals=EVDS_GRUPLARI
      .map(g=>getSeriVeri(g.seriler[urun as keyof typeof g.seriler]).deger)
      .filter((v):v is number=>v!==null);
    if(vals.length>0) stats[urun]={min:Math.min(...vals),max:Math.max(...vals)};
  });

  const bgRenk=(urun:string,val:number|null)=>{
    if(val===null||!stats[urun]) return "transparent";
    const {min,max}=stats[urun];
    if(min===max) return "#F5F6F8";
    const pct=(val-min)/(max-min);
    const isFon=urun==="mevduat"||urun==="katilma";
    const p=isFon?1-pct:pct;
    const r=Math.round(80+p*160);
    const g=Math.round(200-p*140);
  ...
  return(
    <div style={{padding:"0 0 32px"}}>
      {/* Grafik Modal */}
      {grafik&&<OranGrafikModal grup={grafik.grup} urun={grafik.urun} seriKodu={grafik.seriKodu} onClose={()=>setGrafik(null)}/>}

      {/* Başlık */}
      <div style={{background:`linear-gradient(135deg,#1C3A5E 0%,#2C5F8A 100%)`,padding:"16px 16px 14px",marginBottom:12}}>
        <p style={{margin:"0 0 2px",fontSize:11,color:"rgba(255,255,255,0.55)",textTransform:"uppercase",letterSpacing:"0.07em"}}>TCMB EVDS · Haftalık Ağırlıklı Ortalama</p>
        <h2 style={{margin:0,fontSize:18,fontWeight:800,color:"#fff"}}>EVDS Oran Karşılaştırma</h2>
        {veriTarihi&&(
          <p style={{margin:"5px 0 0",fontSize:12,color:"rgba(255,255,255,0.75)",display:"flex",alignItems:"center",gap:5}}>
            <span style={{fontSize:13}}>📅</span>
            <span>Veri: <b>{veriTarihi}</b></span>
          </p>
        )}
      </div>
      ...
      <div style={{padding:"0 12px"}}>
        ...
        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}>
          <button onClick={verileriCek} disabled={yukleniyor} style={{
            ...
            fontSize:13,fontWeight:700,cursor:yukleniyor?"not-allowed":"pointer",
          }}>
        ...
        {hata&&(
          <div style={{background:"#FEF2F2",border:"1px solid #FECACA",borderRadius:10,padding:"10px 14px",marginBottom:10}}>
            ...
            <p style={{margin:"4px 0 0",fontSize:11,color:"#B83232"}}>EVDS_KEY env değişkenini kontrol edin.</p>
          </div>
        )}

        <div style={{background:C.blueLight,borderRadius:10,padding:"8px 12px",marginBottom:10,display:"flex",gap:8,alignItems:"flex-start"}}>
          <span style={{fontSize:13}}>ℹ️</span>
          <p style={{margin:0,fontSize:11,color:C.blue,lineHeight:1.55}}>
            Yıllık basit faiz (%). Kredi için <b>düşük</b>, mevduat/katılma için <b>yüksek</b> değerler yeşil.
            Hücreye tıklayınca <b>trend grafiği</b> açılır. Başlığa tıklayarak sıralayın.
          </p>
        </div>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
              <thead>
                <tr style={{background:"#F0F4F8"}}>
                  <th style={{
                    padding:"10px 10px",textAlign:"left",fontWeight:700,color:C.label,
                    fontSize:11,whiteSpace:"nowrap",position:"sticky",left:0,background:"#F0F4F8",zIndex:2,
                    borderBottom:`2px solid ${C.border}`,
                  }}>Banka Grubu</th>
                  {Object.entries(EVDS_URUNLER).map(([urun,{label,icon,renk}])=>(
                    <th key={urun} onClick={()=>handleSirala(urun)} style={{
                      padding:"8px 6px",textAlign:"center",fontWeight:700,
                      color:siralamaSutun===urun?renk:C.label,
                      fontSize:10,whiteSpace:"nowrap",cursor:"pointer",
                      borderBottom:`2px solid ${C.border}`,minWidth:68,
                    }}>
                      <div>{icon}</div>
                      <div>{label}</div>
                      {siralamaSutun===urun&&<div style={{fontSize:9}}>{siralamaAsc?"↑ Artan":"↓ Azalan"}</div>}
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {siraliGruplar.map((grup,i)=>(
                  <tr key={grup.id} style={{borderTop:`1px solid ${C.border}`,background:i%2===0?"#fff":"#FAFBFC"}}>
                    <td style={{padding:"10px 10px",position:"sticky",left:0,background:i%2===0?"#fff":"#FAFBFC",zIndex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:28,height:28,borderRadius:8,background:grup.bg,fontSize:14,flexShrink:0}}>{grup.icon}</span>
                        <span style={{fontWeight:700,color:grup.renk,fontSize:11,lineHeight:1.3}}>{grup.label}</span>
                      </div>
                    </td>
                    {Object.keys(EVDS_URUNLER).map(urun=>{
                      const seriKodu=grup.seriler[urun as keyof typeof grup.seriler];
                      const {deger:val,tarih:vTarih}=getSeriVeri(seriKodu);
                      const bg=bgRenk(urun,val);
                      const {min}=stats[urun]||{};
                      const {max}=stats[urun]||{};
                      const isFon=urun==="mevduat"||urun==="katilma";
                      const isEnIyi=val!==null&&(isFon?val===max:val===min);
                      const tiklanabilir=seriKodu!==null&&val!==null;
                      // Hafta tarihi kısa format
                      const haftaEtiketi=(()=>{
                        if(!vTarih) return null;
                        const [g,m]=vTarih.split("-");
                        const AYLAR_K=["Oca","Şub","Mar","Nis","May","Haz","Tem","Ağu","Eyl","Eki","Kas","Ara"];
                        return `${parseInt(g)} ${AYLAR_K[parseInt(m)-1]||m}`;
                      })();
                      return(
                        <td key={urun} style={{padding:"5px 4px",textAlign:"center"}}>
                          {seriKodu===null?(
                            ...
                            <div
                              onClick={()=>tiklanabilir&&setGrafik({grup,urun,seriKodu})}
                              style={{
                                background:bg,borderRadius:8,padding:"5px 3px 3px",
                                fontWeight:700,fontSize:12,color:C.label,
                                border:isEnIyi?`2px solid ${EVDS_URUNLER[urun].renk}`:`2px solid transparent`,
                                cursor:tiklanabilir?"pointer":"default",
                                display:"flex",flexDirection:"column",alignItems:"center",gap:1,
                              }}>
                              <div style={{display:"flex",alignItems:"center",gap:2}}>
                                {isEnIyi&&<span style={{fontSize:9}}>★</span>}
                                <span>{fmtOran(val)}</span>
                              </div>
                              {haftaEtiketi&&(
                                <span style={{fontSize:8,fontWeight:500,color:C.sub,opacity:0.8}}>
                                  {haftaEtiketi}
                                </span>
                              )}
                            </div>
        ...
        <div style={{marginTop:8,padding:"8px 12px",background:"#F8FAFC",borderRadius:10,border:`1px solid ${C.border}`}}>
          <p style={{margin:"0 0 4px",fontSize:10,fontWeight:700,color:C.sub}}>EVDS Seri Kodları</p>
          <div style={{display:"flex",flexWrap:"wrap",gap:"2px 12px"}}>
            {[
              ["Konut Tüm","TP.KTF10"],["Konut Kamu","TP.KTF101"],["Konut Özel","TP.KTF102"],
              ["Taşıt Tüm","TP.KTF11"],["İhtiyaç Tüm","TP.KTF12"],
              ["Katılım Konut","TP.KTF17"],["Mevduat 1-3A","TP.TL.MT3"],["Katılma 1-3A","TP.TL.MT3.KT"],
            ].map(([ad,kod])=>(
              <span key={kod} style={{fontSize:9,color:C.sub}}><b>{ad}:</b> {kod}</span>
            ))}
          </div>
        </div>
...
const MENU = {
  ...
  fonGetiriIzleme:{title:"Fon Getiri İzleme ve Hesaplama",back:"home"},
  bistHisseTarayici:{title:"BIST Veri İzleme",back:"home"},
...
{/* ── KATILIM MENU ── */}
        ...
        {/* ── YASAL UYARI FOOTER ── */}
        {!["home","katilimMenu","finansmanMenu","ticariMenu","asistan","sozluk","finansalTakvim","finansalGostergeler","oranKarsilastirma","ayarlar"].includes(screen)&&(
          <div style={{
