...
function BistHisseTarayici() {
  ...
  const [siraDir, setSiraDir]           = useState<1|-1>(1);
  ...
  return (
    <div style={{background:C.bg, padding:"12px 14px 80px", minHeight:"100%"}}>
      ...
      <div style={{display:"flex",gap:4,marginBottom:10,overflowX:"auto"}}>
        {([["fk","F/K"],["pddd","PD/DD"],["roe","ROE"],["temetu","Temettü"]] as [typeof siraBy, string][]).map(([col, lbl]) => (
      ...
      {yukleniyor ? (
        ...
        <div>
          ...
          {filtreli.map((h, i) => (
            <div key={h.ticker}>
              <div
                onClick={() => setSecilen(secilen?.ticker === h.ticker ? null : h)}
                style={{
...
const MENU = {
  ...
  bistHisseTarayici:{title:"BIST Hisse Tarayıcı",back:"home"},
      ...
      <div style={{paddingTop:0}}>
        ...
        {screen==="home"&&(
          <div style={{background:"#0F1923",height:"calc(100vh - 112px)",padding:"8px 10px 0",boxSizing:"border-box",display:"flex",flexDirection:"column",position:"relative",overflowY:"auto"}}>
            ...
            {/* BIST Hisse Tarayıcı — tam genişlik */}
            <div onClick={()=>nav("bistHisseTarayici")} style={{
              background:"linear-gradient(145deg,#1A3A5E 0%,#2A5A8A 100%)",
              borderRadius:18, cursor:"pointer", padding:"14px 20px",
              display:"flex", alignItems:"center", justifyContent:"space-between",
              boxShadow:"0 4px 16px #1A3A5E88", marginBottom:8,
            }}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontSize:20}}>📊</span>
                <div>
                  <span style={{fontSize:15,fontWeight:800,color:"#fff"}}>BIST Hisse Tarayıcı</span>
                  <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",marginTop:2}}>F/K · PD/DD · ROE · Katılım Endeksi</div>
                </div>
              </div>
