      ...
      <div style={{marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
        ...
        >
          ...
          {YONETICILER.map(y=>{
            ...
            return <option key={y} value={y}>{kisaYon(y)} ({sayi})</option>;
          })}
        </select>
      </div>
...
function HisseDetay({ hisse, onGeri }: { hisse: any, onGeri: () => void }) {
  ...
  useEffect(() => {
    ...
    const bitis = bugun.toLocaleDateString("tr-TR", {day:"2-digit",month:"2-digit",year:"numeric"}).split(".").join("-");
    ...
    const bas = baslangic.toLocaleDateString("tr-TR", {day:"2-digit",month:"2-digit",year:"numeric"}).split(".").join("-");

    fetch(`https://www.isyatirim.com.tr/_layouts/15/Isyatirim.Website/Common/Data.aspx/HisseTekil?hisse=${hisse.ticker}&startdate=${bas}&enddate=${bitis}.json`)
      .then(r => r.json())
      .then(d => {
    ...
    <div style={{background:C.bg, padding:"12px 14px 80px", minHeight:"100%"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            <div style={{fontSize:15,fontWeight:700,color:C.text}}>BIST Hisse Tarayıcı</div>
            <div style={{fontSize:11,color:C.sub}}>{filtreli.length} hisse · Fonoloji</div>
          </div>
        </div>
        <div style={{textAlign:"right"}}>
