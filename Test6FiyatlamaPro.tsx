...
function FonGetiriIzleme({ settings }) {
  ...
  const filtreli = useMemo(()=>{
    ...
    return fonlar
      .filter(f=>{
        ...
        const yOk=filtreYon==="Tümü"||f.yonetici.trim()===filtreYon;
        return aOk&&yOk;
      })
      .sort((a,b)=>{
  ...
  return (
    <div style={s.wrap}>
      ...
      {/* Yönetici filtresi — dropdown */}
      <div style={{marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:11,color:FC.sub,flexShrink:0}}>Portföy Yöneticisi:</span>
        <select
          value={filtreYon}
          onChange={e=>setFiltreYon(e.target.value)}
          style={{
            flex:1,
            background:FC.card,
        ...
        >
          <option value="Tümü">Tümü ({fonlar.length} fon)</option>
          {YONETICILER.map(y=>{
            const sayi = fonlar.filter(f=>f.yonetici.trim()===y).length;
            return <option key={y} value={y}>{y} ({sayi})</option>;
          })}
        </select>
      </div>
      ...
      {/* Tablo başlığı */}
      <div style={s.thRow}>
        <span style={{width:44,flexShrink:0}}>Fon Kodu</span>
        <span style={{flex:1}}>Fon Adı</span>
        <span style={{width:76,textAlign:"right",flexShrink:0}}>{periodLabel} Getiri</span>
        <span style={{width:70,textAlign:"right",flexShrink:0}}>Yıllık Getiri</span>
        <span style={{width:72,textAlign:"right",flexShrink:0}}>Fon Büyüklüğü</span>
      </div>
                ...
                <div key={fon.kod}>
                  ...
                  <div onClick={()=>handleFonTikla(fon)} style={{
                    ...
                    <div style={s.fonL}>
                      ...
                      <span style={{...s.fKod,color:vakif?FC.green:FC.blue,width:40,flexShrink:0}}>{fon.kod}{vakif&&<span style={{fontSize:8,color:FC.green,opacity:0.8}}> ★</span>}</span>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:12,fontWeight:700,color:"#fff",lineHeight:1.3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{fon.ad}</div>
                        <span style={s.fYon}>{fon.yonetici}</span>
                        <div style={s.barWrap}>
