        ...
        <div key={gi} style={{marginBottom:6}}>
          ...
          <div style={{background:"#fff",borderRadius:14,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.07)"}}>
            {grup.items.map((item,ii)=>{
              ...
              return(
                <div key={ii} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 16px",borderBottom:ii<grup.items.length-1?"1px solid #F0F4F8":"none"}}>
                  <div>
  ...
  useEffect(()=>{
    const loadKurlar = async () => {
      ...
      if(d){
        ...
        // Bitcoin ekle
        try{
          const kripto = await fetch('/api/kripto').then(r=>r.ok?r.json():null).catch(()=>null);
          if(kripto?.btc_try) liste.push({kod:"BTC", try: kripto.btc_try, kripto:true});
        }catch(e){}
        setKurlar(liste);
        setKurYuklendi(true);
      }
              ...
              <div className="ticker-inner">
                {[...kurlar,...kurlar].map((k,i)=>(
                  <span key={i} style={{
                    ...
                    <span style={{color:"rgba(255,255,255,0.45)",fontSize:10}}>{k.altin||k.kripto ? k.kod : k.kod+"/TRY"}</span>
                    <span style={{color:kurYuklendi?(k.kripto?"#F7931A":k.altin?"#FFD60A":"#4ADE80"):"rgba(255,255,100,0.7)",fontFamily:"monospace"}}>{typeof k.try==="number"?fmtN(k.try,k.kripto?0:2):k.try}</span>
                  </span>
                ))}
              </div>
            </div>
